...
nvVehicleDectriptor::ndEngineTorqueCurve::ndEngineTorqueCurve()
{
	// take from the data sheet of a 2005 dodge viper, 
	// some values are missing so I have to improvise them
	dFloat32 fuelInjectionRate = 10.0f;
...
void nvVehicleDectriptor::ndEngineTorqueCurve::Init(
...
dFloat32 nvVehicleDectriptor::ndEngineTorqueCurve::GetFuelRate() const
{
	return m_fuelInjectionRate;
}

dFloat32 nvVehicleDectriptor::ndEngineTorqueCurve::GetIdleRadPerSec() const
{
	return m_torqueCurve[1].m_radPerSeconds;
}

dFloat32 nvVehicleDectriptor::ndEngineTorqueCurve::GetLowGearShiftRadPerSec() const
{
	return m_torqueCurve[2].m_radPerSeconds;
}

dFloat32 nvVehicleDectriptor::ndEngineTorqueCurve::GetHighGearShiftRadPerSec() const
{
	return m_torqueCurve[3].m_radPerSeconds;
}

dFloat32 nvVehicleDectriptor::ndEngineTorqueCurve::GetRedLineRadPerSec() const
{
	const int maxIndex = sizeof(m_torqueCurve) / sizeof(m_torqueCurve[0]);
	return m_torqueCurve[maxIndex - 1].m_radPerSeconds;
}

dFloat32 nvVehicleDectriptor::ndEngineTorqueCurve::GetTorque(dFloat32 omegaInRadPerSeconds) const
{
	const int maxIndex = sizeof(m_torqueCurve) / sizeof(m_torqueCurve[0]);
	omegaInRadPerSeconds = dClamp(omegaInRadPerSeconds, dFloat32(0.0f), m_torqueCurve[maxIndex - 1].m_radPerSeconds);
...
nvVehicleDectriptor::nvVehicleDectriptor(const char* const fileName)
	:m_comDisplacement(dVector::m_zero)
{
	strncpy(m_name, fileName, sizeof(m_name));
...
class ndBasicVehicle : public ndMultiBodyVehicle
{
	...
	{
		ndBasicVehicle* const me = (ndBasicVehicle*)context;
		me->RenderUI(scene);
	}
