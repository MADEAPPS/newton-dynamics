...
//	dVector* const normals = new dVector [size * size];
	...
	for (int z = 0; z < size - 1; z ++) {
		for (int x = 0; x < size - 1; x ++) {
			dVector p0 ((x + 0) * cellSize, elevationMap[z + 0][x + 0], (z + 0) * cellSize, 0.0f);
			dVector p1 ((x + 1) * cellSize, elevationMap[z + 0][x + 1], (z + 0) * cellSize, 0.0f);
			dVector p2 ((x + 1) * cellSize, elevationMap[z + 1][x + 1], (z + 1) * cellSize, 0.0f);
			dVector p3 ((x + 0) * cellSize, elevationMap[z + 1][x + 0], (z + 1) * cellSize, 0.0f);
...
void DemoBezierCurve::Render (ndDemoEntityManager* const scene)
{
	if (m_isVisible) 
	{
		glDisable (GL_LIGHTING);
		...
		dFloat3264 scale = 1.0f / m_renderResolution;
		glBegin(GL_LINES);
...
DemoSkinMesh::DemoSkinMesh(dScene* const scene, ndDemoEntity* const owner, dScene::dTreeNode* const meshNode, const dTree<ndDemoEntity*, dScene::dTreeNode*>& boneMap, const ShaderPrograms& shaderCache)
	:ndDemoMeshInterface()
...
DemoSkinMesh::DemoSkinMesh(const DemoSkinMesh& clone, ndDemoEntity* const owner)
...
ndDemoMeshInterface* DemoSkinMesh::Clone(ndDemoEntity* const owner)
{
	return (DemoSkinMesh*)new DemoSkinMesh(*this, owner);
}

void DemoSkinMesh::OptimizeForRender(const ndDemoSubMesh& segment, const dVector* const pointWeights, const dWeightBoneIndex* const pointSkinBone) const
{
...
void DemoSkinMesh::Render (ndDemoEntityManager* const scene)
{
