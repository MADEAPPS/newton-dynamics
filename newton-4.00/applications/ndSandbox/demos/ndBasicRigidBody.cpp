...
static ndMatrix OrthoInvert(const ndMatrix& mFrame)
{
    ...
    mRet.m_front = ndVector(mFrame.m_front[0], mFrame.m_up[0], mFrame.m_right[0], 0.0);
    mRet.m_up = ndVector(mFrame.m_front[1], mFrame.m_up[1], mFrame.m_right[1], 0.0);
    mRet.m_right = ndVector(mFrame.m_front[2], mFrame.m_up[2], mFrame.m_right[2], 0.0);
    mRet.m_posit = ndVector(-mFrame.m_front[0] * mFrame.m_posit.GetX() - mFrame.m_front[1] * mFrame.m_posit.GetY() - mFrame.m_front[2] * mFrame.m_posit.GetZ(),
        ...
        -mFrame.m_right[0] * mFrame.m_posit.GetX() - mFrame.m_right[1] * mFrame.m_posit.GetY() - mFrame.m_right[2] * mFrame.m_posit.GetZ(), 1.0);
...
static void CreateHinge(ndPhysicsWorld* const pWorld, ndBodyKinematic* const pBodyA, ndBodyKinematic* pBodyB, const ndVector& vPivotA, const ndVector& vPivotB, const ndVector& vAxisA, const ndVector& vAxisB)
{
    ...
    mFrameA.m_front = ndVector(vAx.GetX(), vAx.GetY(), vAx.GetZ(), 0.0);
    mFrameA.m_up = ndVector(vAy.GetX(), vAy.GetY(), vAy.GetZ(), 0.0);
    mFrameA.m_right = ndVector(vAz.GetX(), vAz.GetY(), vAz.GetZ(), 0.0);

    ndMatrix mFrameB = ndGetIdentityMatrix();
    ndVector vBx = vAxisB, vBy, vBz;
    BuildOrthoNormalBasis(vBy, vBz, vBx);
    mFrameB.m_posit = vPivotB;
    mFrameB.m_front = ndVector(vBx.GetX(), vBx.GetY(), vBx.GetZ(), 0.0);
    mFrameB.m_up = ndVector(vBy.GetX(), vBy.GetY(), vBy.GetZ(), 0.0);
    mFrameB.m_right = ndVector(vBz.GetX(), vBz.GetY(), vBz.GetZ(), 0.0);
...
void ndBasicRigidBody(ndDemoEntityManager* const scene)
{
    ...
    origin1.m_posit = ndVector(0.0, 8.0, 0.0, 1.0);

    // Bodies
    auto pMainBody = AddBox(scene, origin1, 1500000, 4.0, 8.0, 8.0);
