...
static void AddBox(ndDemoEntityManager* const scene, const dVector& origin, dFloat32 density, int count)
{
	ndShapeInstance shape(new ndShapeBox(1.0f, 2.0f, 0.7f));
...
void ndBasicGpuRigidBody(ndDemoEntityManager* const scene)
{
	...
	dInt32 count = 1;
	AddBox(scene, dVector(0.0f, 2.0f, -3.0f, 1.0f), 1.0f, count);

	dQuaternion rot;
	dVector origin(-10.0f - 5.0f * count, 5.0f, 0.0f, 0.0f);
