...
{
	public:
	ndIndustrialRobot(ndDemoEntityManager* const scene, fbxDemoEntity* const ragdollMesh, const ndMatrix& location)
		:ndModel()
	{
		dAssert(0);
...
void ndInsdustrialRobot (ndDemoEntityManager* const scene)
{
	...
	//ndIndustrialRobot* const ragdoll = new ndIndustrialRobot(scene, ragdollMesh, matrix);
	//scene->SetSelectedModel(ragdoll);
	//scene->GetWorld()->AddModel(ragdoll);
	//
	//matrix.m_posit.m_x += 2.0f;
	//matrix.m_posit.m_z -= 2.0f;
	//scene->GetWorld()->AddModel(new ndIndustrialRobot(scene, ragdollMesh, matrix));
