...
class ndAiBipedTest_2_Notify : public ndDemoEntityNotify
{
	public:
	ndAiBipedTest_2_Notify(ndDemoEntityManager* const manager, ndDemoEntity* const entity, ndBodyDynamic* const parentBody)
...
class ndAiBipedTest_2 : public ndModel
{
		...
		:ndModel()
	{
		...
		// set bindimg matrix;
		ndAiBipedTest_2_Notify* const rootNotify = (ndAiBipedTest_2_Notify*)rootBody->GetNotifyCallback();
		rootNotify->m_bindMatrix = rootEntity->GetParent()->CalculateGlobalMatrix().Inverse();
		rootNotify->OnTransform(0, matrix);
	...
	ndBodyDynamic* CreateBodyPart(ndDemoEntityManager* const scene, ndDemoEntity* const entityPart, ndBodyDynamic* const parentBone)
	{
		...
		body->SetNotifyCallback(new ndAiBipedTest_2_Notify(scene, entityPart, parentBone));
