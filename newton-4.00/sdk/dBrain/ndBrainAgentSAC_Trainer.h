...
#ifndef _ND_BRAIN_AGENT_SAC_TRAINER_H__
#define _ND_BRAIN_AGENT_SAC_TRAINER_H__
...
#define D_SAC_CRITIC_LEARN_RATE		ndReal(0.001f)
#define D_SAC_ACTOR_LEARN_RATE		ndReal(0.0001f)
#define D_SAC_DISCOUNT_FACTOR		ndReal (0.99f)
#define D_SAC_REPLAY_BUFFERSIZE		(1024 * 512)
//#define D_SAC_REPLAY_BUFFERSIZE	(1024)
//#define D_SAC_REPLAY_BASH_SIZE	32
#define D_SAC_REPLAY_BASH_SIZE		64
#define D_SAC_REGULARIZER			ndReal (2.0e-6f)
#define D_SAC_SOFT_TARGET_FACTOR	ndReal (1.0e-3f)
#define D_SAC_ACTION_NOISE_VARIANCE	ndReal (0.05f)

template<ndInt32 statesDim, ndInt32 actionDim>
class ndBrainAgentSAC_Trainer : public ndBrainAgent, public ndBrainThreadPool
{
	public:
	ndBrainAgentSAC_Trainer(const ndSharedPtr<ndBrain>& actor, const ndSharedPtr<ndBrain>& critic);
	~ndBrainAgentSAC_Trainer();
...
template<ndInt32 statesDim, ndInt32 actionDim>
ndBrainAgentSAC_Trainer<statesDim, actionDim>::ndBrainAgentSAC_Trainer(const ndSharedPtr<ndBrain>& actor, const ndSharedPtr<ndBrain>& critic)
	:ndBrainAgent()
	,ndBrainThreadPool()
	,m_actor(actor)
	,m_critic0(critic)
	...
	,m_replayBuffer()
	,m_currentTransition()
	,m_gamma(D_SAC_DISCOUNT_FACTOR)
	,m_currentQValue(ndReal(0.0f))
	,m_actorLearnRate(D_SAC_ACTOR_LEARN_RATE)
	,m_criticLearnRate(D_SAC_CRITIC_LEARN_RATE)
	,m_softTargetFactor(D_SAC_SOFT_TARGET_FACTOR)
	,m_actionNoiseVariance(D_SAC_ACTION_NOISE_VARIANCE)
	,m_frameCount(0)
	,m_framesAlive(0)
	,m_eposideCount(0)
	,m_bashBufferSize(D_SAC_REPLAY_BASH_SIZE)
	,m_collectingSamples(true)
{
	...
	for (ndInt32 i = 0; i < ndBrainThreadPool::GetThreadCount(); ++i)
	{
		...
		m_actorOptimizer[m_actorOptimizer.GetCount() - 1]->SetRegularizer(D_SAC_REGULARIZER);
		m_criticOptimizer0[m_criticOptimizer0.GetCount() - 1]->SetRegularizer(D_SAC_REGULARIZER);
		m_criticOptimizer1[m_criticOptimizer1.GetCount() - 1]->SetRegularizer(D_SAC_REGULARIZER);
	}

	SetBufferSize(D_SAC_REPLAY_BUFFERSIZE);
	InitWeights();
}

template<ndInt32 statesDim, ndInt32 actionDim>
ndBrainAgentSAC_Trainer<statesDim, actionDim>::~ndBrainAgentSAC_Trainer()
{
}

template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::Save(ndBrainSave* const loadSave) const
{
	loadSave->Save(*m_actor);
}

template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::SetBufferSize(ndInt32 size)
{
	m_replayBuffer.SetSize(size);
}

template<ndInt32 statesDim, ndInt32 actionDim>
bool ndBrainAgentSAC_Trainer<statesDim, actionDim>::IsTrainer() const
{
	return true;
}

template<ndInt32 statesDim, ndInt32 actionDim>
bool ndBrainAgentSAC_Trainer<statesDim, actionDim>::IsSampling() const
{
	return m_collectingSamples;
}

template<ndInt32 statesDim, ndInt32 actionDim>
ndReal ndBrainAgentSAC_Trainer<statesDim, actionDim>::GetCurrentValue() const
{
	return m_currentQValue;
}

template<ndInt32 statesDim, ndInt32 actionDim>
ndInt32 ndBrainAgentSAC_Trainer<statesDim, actionDim>::GetFramesCount() const
{
	return m_frameCount;
}

template<ndInt32 statesDim, ndInt32 actionDim>
ndInt32 ndBrainAgentSAC_Trainer<statesDim, actionDim>::GetEposideCount() const
{
	return m_eposideCount;
}

template<ndInt32 statesDim, ndInt32 actionDim>
ndInt32 ndBrainAgentSAC_Trainer<statesDim, actionDim>::GetEpisodeFrames() const
{
	return m_framesAlive;
}

template<ndInt32 statesDim, ndInt32 actionDim>
ndReal ndBrainAgentSAC_Trainer<statesDim, actionDim>::GetActionNoise() const
{
	return m_actionNoiseVariance;
}

template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::SetActionNoise(ndReal noiseVariance)
{
	m_actionNoiseVariance = noiseVariance;
}

template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::InitWeights()
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::InitWeights(ndReal weighVariance, ndReal biasVariance)
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
bool ndBrainAgentSAC_Trainer<statesDim, actionDim>::IsTerminal() const
{
	ndAssert(0);
	return false;
}
...
template<ndInt32 statesDim, ndInt32 actionDim>
ndReal ndBrainAgentSAC_Trainer<statesDim, actionDim>::GetReward() const
{
	ndAssert(0);
	return ndReal(0.0f);
}
...
template<ndInt32 statesDim, ndInt32 actionDim>
ndReal ndBrainAgentSAC_Trainer<statesDim, actionDim>::PerturbeAction(ndReal action) const
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::PopulateReplayBuffer()
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::BackPropagateCritic(const ndUnsigned32* const shuffleBuffer)
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::BackPropagateActor(const ndUnsigned32* const shuffleBuffer)
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::BackPropagate()
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::Optimize()
{
	BackPropagate();
	if (IsSampling())
	{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::CalculateQvalue(const ndBrainVector& state, const ndBrainVector& actions)
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::Step()
{
...
template<ndInt32 statesDim, ndInt32 actionDim>
void ndBrainAgentSAC_Trainer<statesDim, actionDim>::OptimizeStep()
{
	if (!m_frameCount)
	{
		ResetModel();
