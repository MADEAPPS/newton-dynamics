...
ndTrainerDescriptor::ndTrainerDescriptor(const ndSharedPtr<ndBrain>& brain, const ndSharedPtr<ndBrainContext>& context, ndInt32 minibatchSize, ndBrainFloat learnRate)
	...
	,m_regularizer(ndBrainFloat(1.0e-4f))
	,m_minibatchSizeExp(minibatchSize)
	,m_regularizerType(m_ridge)
{
}
...
{
	ndAssert((m_descriptor.m_minibatchSizeExp & (ND_GPU_TILED_MATRIX_ROWS - 1)) == 0);
...
void ndBrainTrainerInference::InitInputOutputBuffer()
{
	...
	buffer.Resize(bufferSize * m_descriptor.m_minibatchSizeExp);
	buffer.SetCount(bufferSize * m_descriptor.m_minibatchSizeExp);
	...
	//copyBufferInfo.m_dstStrideInByte = copyBufferInfo.m_strideInByte * m_descriptor.m_minibatchSizeExp;
...
void ndBrainTrainerInference::InitWeightAndBiasBuffer()
{
	...
	scratchBuffer.SetCount(m_descriptor.m_minibatchSizeExp * brain.GetInputSize());
	scratchBuffer.Set(ndBrainFloat(0.0f));
	m_miniBatchInputBuffer = ndSharedPtr<ndBrainFloatBuffer>(new ndBrainFloatBuffer(*m_descriptor.m_context, scratchBuffer, true));

	scratchBuffer.SetCount(m_descriptor.m_minibatchSizeExp * brain.GetOutputSize());
	scratchBuffer.Set(ndBrainFloat(0.0f));
...
void ndBrainTrainerInference::AddLayersCommands(ndFixSizeArray<ndCommandSharedInfo, 256>& layersUniformsData)
{
	...
	for (ndInt32 i = 0; i < ndInt32(brain.GetCount()); ++i)
	{
		...
		ndCommandArray backCommands(layer->CreateGpuFeedForwardCommand(
			this, *m_descriptor.m_context, uniformParam, m_descriptor.m_minibatchSizeExp, inputOutputBuffer, weightsBuffer));
