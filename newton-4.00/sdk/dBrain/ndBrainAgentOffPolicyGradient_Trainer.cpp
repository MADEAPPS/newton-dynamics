...
void ndBrainAgentOffPolicyGradient_Trainer::CalculateExpectedRewards()
{
	...
	if (m_parameters.m_useSofActorCritic)
	{
		...
		m_minibatchMean->Set(m_minibatchMeanAction);


		// add noise to the actions mean and clip the result to the actions limits
		m_minibatchMeanAction->Add(**m_minibatchUniformRandomDistribution);
		m_minibatchMeanAction->Min(ndBrainFloat(1.0f));
		m_minibatchMeanAction->Max(ndBrainFloat(-1.0f));
