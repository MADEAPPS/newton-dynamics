...
#define ND_CONTINUE_POLICY_PROXIMA_ITERATIONS		5
#define ND_CONTINUE_POLICY_KL_DIVERGENCE			ndBrainFloat(0.01f)
#define ND_CONTINUE_POLICY_CLIP_EPSILON				ndBrainFloat(0.1f)
			...
			class MaxLikelihoodLoss : public ndBrainLoss
			{
				...
				void GetLoss(const ndBrainVector& probabilityDistribution, ndBrainVector& loss)
				{
					...
					for (ndInt32 i = numberOfActions - 1; i >= 0; --i)
					{
						...
						ndFloat32 prob_q = sampledProbability[i] - probabilityDistribution[i];
						ndFloat32 sigma_q = probabilityDistribution[i + numberOfActions];
						exp_q += prob_q * sigma_q * prob_q;
						det_q *= probabilityDistribution[i + numberOfActions];
					}
					...
					ndBrainFloat g = (rawAdvantage > ndBrainFloat(0.0f)) ? ndBrainFloat(1.0 + ND_CONTINUE_POLICY_CLIP_EPSILON) : ndBrainFloat(1.0 - ND_CONTINUE_POLICY_CLIP_EPSILON);
