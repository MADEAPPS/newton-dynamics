...
//#define DG_CYLINDER_SEGMENTS		10
//#define DG_CYLINDER_CAP_SEGMENTS	12
...
void ndShapeCylinder::Init(dFloat32 radio0, dFloat32 radio1, dFloat32 height)
{
	...
	//}
	//
	//dVector tempVertex[4 * DG_CYLINDER_CAP_SEGMENTS * DG_CYLINDER_SEGMENTS + 100];
	//dInt32 index = 0;
	//dInt32 dx0 = dInt32(dFloor(DG_CYLINDER_SEGMENTS * ((m_p0.m_x + m_height + m_radius0) / m_radius0)) + dFloat32(1.0f));
	//dFloat32 step = m_radius0 / DG_CYLINDER_SEGMENTS;
	...
	//	for (dInt32 i = 0; i < DG_CYLINDER_CAP_SEGMENTS; i++) 
	//	{
	//		dFloat32 z = dSin(angle);
	//		dFloat32 y = dCos(angle);
	//		tempVertex[index] = dVector(x0, y * r0, z * r0, dFloat32(0.0f));
	//		index++;
	//		angle += (dFloat32(2.0f) * dPi) / DG_CYLINDER_CAP_SEGMENTS;
	//		dAssert(index < sizeof(tempVertex) / sizeof(tempVertex[0]));
	//	}
	//}
	//
	//dFloat32 x1 = m_p1.m_x;
	//dInt32 dx1 = dInt32(dFloor(DG_CYLINDER_SEGMENTS * ((m_height + m_radius1 - m_p1.m_x) / m_radius1)) + dFloat32(1.0f));
	//step = m_radius1 / DG_CYLINDER_SEGMENTS;
	...
	//polyhedra.BeginFace();
	//
	//dInt32 wireframe[DG_CYLINDER_SEGMENTS + 10];
	//
	//dInt32 i1 = 0;
	//dInt32 i0 = DG_CYLINDER_CAP_SEGMENTS - 1;
	//const dInt32 n = index / DG_CYLINDER_CAP_SEGMENTS - 1;
	...
	//		wireframe[0] = i0;
	//		wireframe[1] = i1;
	//		wireframe[2] = i1 + DG_CYLINDER_CAP_SEGMENTS;
	//		wireframe[3] = i0 + DG_CYLINDER_CAP_SEGMENTS;
	//		i0 = i1;
	//		i1++;
	//		polyhedra.AddFace(4, wireframe);
	//	}
	//	i0 = i1 + DG_CYLINDER_CAP_SEGMENTS - 1;
	//}
	//
	//for (dInt32 i = 0; i < DG_CYLINDER_CAP_SEGMENTS; i++) 
	//{
	//	wireframe[i] = DG_CYLINDER_CAP_SEGMENTS - i - 1;
	//}
	//polyhedra.AddFace(DG_CYLINDER_CAP_SEGMENTS, wireframe);
	//
	//for (dInt32 i = 0; i < DG_CYLINDER_CAP_SEGMENTS; i++) 
	//{
	//	wireframe[i] = index - DG_CYLINDER_CAP_SEGMENTS + i;
	//}
	//polyhedra.AddFace(DG_CYLINDER_CAP_SEGMENTS, wireframe);
	//polyhedra.EndFace();
	//
...
ndShapeInfo ndShapeCylinder::GetShapeInfo() const
{
	...
	//{
	//	dSwap(info.m_cylinder.m_radio0, info.m_cylinder.m_radio1);
	//}
	return info;
}
...
dVector ndShapeCylinder::SupportVertexSpecialProjectPoint(const dVector& testPoint, const dVector& direction) const
{
	dAssert(0);
...
dVector ndShapeCylinder::SupportVertex(const dVector& direction, dInt32* const vertexIndex) const
{
	dAssert(0);
	return dVector::m_zero;
...
dVector ndShapeCylinder::SupportVertexSpecial(const dVector& direction, dFloat32 skinThickness, dInt32* const vertexIndex) const
{
	dAssert(0);
	return dVector::m_zero;
...
dInt32 ndShapeCylinder::CalculatePlaneIntersection(const dVector& direction, const dVector& point, dVector* const contactsOut) const
{
	dAssert(0);
	return 0;
