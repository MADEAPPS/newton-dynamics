...
class ntShapeInstance: public dClassAlloc
{
	...
	ND_COLLISION_API ntShapeInstance(ntShape* const shape);
	ND_COLLISION_API ntShapeInstance(const ntShapeInstance& instance);
	...
	ND_COLLISION_API ~ntShapeInstance();
	ND_COLLISION_API ntShapeInstance& operator=(const ntShapeInstance& src);
	...
	ND_COLLISION_API void DebugShape(const dMatrix& matrix, ntShapeDebugCallback& debugCallback) const;

	ND_COLLISION_API dFloat32 RayCast(ntRayCastNotify& callback, const dVector& localP0, const dVector& localP1, dFloat32 maxT, const ntBody* const body, ntContactPoint& contactOut) const;
