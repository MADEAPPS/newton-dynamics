...
dVector ndShapeSphere::m_unitSphere[DG_SPHERE_VERTEX_COUNT];
dgCollisionConvex::dgConvexSimplexEdge ndShapeSphere::m_edgeArray[DG_SPHERE_EDGE_COUNT];
...
void ndShapeSphere::Init (dFloat32 radius, dgMemoryAllocator* allocator)
{
	m_rtti |= ndShapeSphere_RTTI;
	m_radius = dgMax (dAbs (radius), D_MIN_CONVEX_SHAPE_SIZE);
	...
	if (!m_shapeRefCount) {
		...
		//dAssert (count == EDGE_COUNT);
		dInt32 vertexCount = dgVertexListToIndexList (&tmpVectex[0].m_x, sizeof (dVector), 3 * sizeof (dFloat32), 0, count, indexList, 0.001f); 

		dAssert (vertexCount == DG_SPHERE_VERTEX_COUNT);
	...
	for (dInt32 i = 0; i < DG_SPHERE_VERTEX_COUNT; i ++) {
...
void ndShapeSphere::MassProperties()
{
	...
	//dFloat32 volume = dFloat32(4.0f * dgPi / 3.0f) * m_radius *  m_radius * m_radius;
