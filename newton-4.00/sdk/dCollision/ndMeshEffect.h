...
//	void ReverseMergeFaces (ndMeshEffect* const source);
	...
	class ndMaterial
	{
		...
		ndMaterial()
			...
			,m_reflection(ndFloat32(0.5f), ndFloat32(0.5f), ndFloat32(0.5f), ndFloat32(1.0f))
	...
	D_COLLISION_API void CylindricalMapping(ndInt32 materialIndex, const ndMatrix& textureMatrix);
	...
	D_COLLISION_API void DeserializeFromXml(const nd::TiXmlElement* const xmlNode);
	D_COLLISION_API void SerializeToXml(nd::TiXmlElement* const xmlNode, const ndTree<ndString, ndUnsigned32>& boneMap) const;
