...
void ndScene::AddNode(ndSceneNode* const newNode)
{
	if (m_rootNode)
	{
		m_rootNode = new ndSceneTreeNode(m_rootNode, newNode);
		dAssert(m_rootNode->GetAsSceneTreeNode());
		m_fitness.AddNode(m_rootNode->GetAsSceneTreeNode());
	}
	else
	{
		m_rootNode = newNode;
...
void ndScene::BuildSmallBvh(ndSceneNode** const parentsArray, ndUnsigned32 bashCount, ndInt32 depthLevel)
{
	auto SmallBhvNodes = ndMakeObject::ndFunction([this, parentsArray, bashCount, depthLevel](ndInt32 threadIndex, ndInt32 threadCount)
	{
		...
		auto MakeTwoNodesTree = [](ndSceneTreeNode* const root, ndSceneNode* const left, ndSceneNode* const right, ndInt32 depthLevel)
		{
			left->m_bhvLinked = 1;
			right->m_bhvLinked = 1;
			root->m_bhvLinked = 1;
			root->m_depthLevel = depthLevel;
		...
		auto MakeThreeNodesTree = [](ndSceneTreeNode* const root, ndSceneTreeNode* const subRoot, ndSceneNode* const node0, ndSceneNode* const node1, ndSceneNode* const node2, ndInt32 depthLevel)
		{
			class ndNodeOrder
			{
				public:
		...
		for (ndInt32 i = startEnd.m_start; i < startEnd.m_end; ++i)
		{
			...
			if (nodesCount == 1)
			{
				...
				MakeTwoNodesTree(root, node0, node1, depthLevel);
				root->m_bhvLinked = 0;
				root->m_depthLevel = depthLevel;
			}
			else if (nodesCount == 2)
			{
				...
				dAssert(root);
				dAssert(!root->m_bhvLinked);
				MakeThreeNodesTree(root, subParent, node0, node1, node2, depthLevel);
				root->m_bhvLinked = 0;
			}
			else if (nodesCount > 2)
			{
