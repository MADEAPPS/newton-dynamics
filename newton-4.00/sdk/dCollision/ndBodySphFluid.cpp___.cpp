...
void ndBodySphFluid::SortYdimension(ndThreadPool* const threadPool)
{
	D_TRACKTIME();
	...
	class ndKey_low
	{
		...
		ndInt32 GetKey(const ndGridHash& cell) const
		{
			...
			ndUnsigned32 key = ndUnsigned32(m_data.WorldToGrid(m_point[index].m_y));
	...
	for (int i = 1; i < data.m_hashGridMap.GetCount(); ++i)
	{
		...
		ndInt32 key0 = data.WorldToGrid(p0.m_y);
		ndInt32 key1 = data.WorldToGrid(p1.m_y);
		ndAssert(key0 <= key1);
	}
#endif
}
	...
	auto AddPairs = ndMakeObject::ndFunction([this, &data](ndInt32 threadIndex, ndInt32 threadCount)
	{
		...
		auto ProccessCell = [this, &data, &hashGridMap, &pair, &pairCount, &locks, &distance, windowsTest, diameter2](ndInt32 start, ndInt32 count)
		{
			...
			for (ndInt32 i = 0; i < count0; ++i)
			{
				...
				const ndInt32 y0 = data.WorldToGrid(m_posit[particle0].m_y);
				for (ndInt32 j = i + 1; j < count; ++j)
				{
					...
					const ndInt32 y1 = data.WorldToGrid(m_posit[particle1].m_y);
					//ndAssert((x1 - x0) > ndFloat32(-1.0e-3f));
					const ndInt32 sweeptTest = ((y1 - y0) >= windowsTest);
					if (sweeptTest)
					{
						break;
					}
...
void ndBodySphFluid::CaculateAabb(ndThreadPool* const threadPool)
{
	...
	ndInt32 numberOfGrid = ndInt32((box.m_max.m_y - box.m_min.m_y) * invGrid.m_y + ndFloat32(1.0f));
	data.SetWorldToGridMapping(numberOfGrid, m_box1.m_y, m_box0.m_y);
}
