	...
	, m_parent(nullptr)
{
	...
	//	dVector p0 (ndMin (left->m_p0.m_x, right->m_p0.m_x), ndMin (left->m_p0.m_y, right->m_p0.m_y), ndMin (left->m_p0.m_z, right->m_p0.m_z), dFloat32 (0.0f));
	//	dVector p1 (ndMax (left->m_p1.m_x, right->m_p1.m_x), ndMax (left->m_p1.m_y, right->m_p1.m_y), ndMax (left->m_p1.m_z, right->m_p1.m_z), dFloat32 (0.0f));
	...
	while (stack) {
		...
		if (me && dgOverlapTest(me->m_p0, me->m_p1, l0, l1)) {
			...
			if (!me->m_left) {
				...
				stack++;
				ndAssert(stack < ndInt32(sizeof(stackPool) / sizeof(dgMeshBVHNode*)));
		...
		for (iter.Begin(); iter; iter ++) {
			...
			if (!filter.Find(edge)) {
				...
				do {
					...
					ndTrace (("%d ", ptr->m_incidentVertex));
					ptr = ptr->m_next;
				} while (ptr != edge);
			...
			do {
				...
				if (fabs (collinear) > ndFloat64 (1.0e-8f)) {
					...
					if ((va > minError) && (vb > minError) && (vc > minError) && (va < maxError) && (vb < maxError) && (vc < maxError)) {
						...
						ndAssert (fabs (testPoint.m_x - point.m_x) < ndFloat32 (1.0e-2f));
						ndAssert (fabs (testPoint.m_y - point.m_y) < ndFloat32 (1.0e-2f));
						ndAssert (fabs (testPoint.m_z - point.m_z) < ndFloat32 (1.0e-2f));
			#endif
						...
						ndAssert (attr0.m_material == attr1.m_material);
						ndAssert (attr0.m_material == attr2.m_material);
						return attribute;
					}
				}
...
class ndMeshEffect::ndFormat::ndSortCluster
{
	public:
	ndBigVector m_sum;
	ndBigVector m_sum2;
...
inline ndInt32 ndMeshEffect::ndFormat::CompareVertex(const ndSortKey* const ptr0, const ndSortKey* const ptr1, void* const context)
{
	const ndVertexSortData* const sortContext = (ndVertexSortData*)context;
	const ndInt32 compIndex = sortContext->m_vertexSortIndex;
	const ndChannel<ndBigVector, m_point>& points = *sortContext->m_points;
	...
	ndSortKey* const remapIndex, const ndSortCluster& cluster, ndFloat32 tol)
{
	...
	const ndInt32 base = ndInt32 (output.m_vertex.GetCount());
	...
	{
		ndAssert(remapIndex[i].m_attibuteIndex == -1);
...
{
	ndPointFormat tmpFormat(*this);
	Clear();
	const ndInt32 vertexCount = ndInt32 (tmpFormat.m_vertex.GetCount());
	...
	if (vertexCount > ND_VERTEXLIST_INDEX_LIST_BATCH)
	{
		...
		while (stack)
		{
			...
			if ((cluster.m_count <= ND_VERTEXLIST_INDEX_LIST_BATCH) || (stack > (ndInt32 (sizeof(spliteStack) / sizeof(spliteStack[0])) - 4)) || (maxVariance2 < ndFloat32(4.0f)))
			{
...
void ndMeshEffect::ndAttibuteFormat::CompressData(
	ndAttibuteFormat& output, const ndPointFormat& points, ndInt32* const indexList, 
	ndSortKey* const remapIndex, const ndSortCluster& cluster, ndFloat32 tol)
{
	const ndBigVector origin (cluster.m_sum.Scale(ndFloat32(1.0f) / (ndFloat32)cluster.m_count));
	const ndBigVector x2c (cluster.m_sum2.Scale(ndFloat32(1.0f) / (ndFloat32)cluster.m_count) - origin * origin);
		...
		if (ii == -1)
		{
			...
			for (ndInt32 j = i + 1; j < cluster.m_count; ++j)
			{
				...
				if (jj == -1)
				{
					...
					{
						const ndVector colorI(m_colorChannel[i0].m_x, m_colorChannel[i0].m_y, m_colorChannel[i0].m_z, m_colorChannel[i0].m_w);
						const ndVector colorJ(m_colorChannel[j0].m_x, m_colorChannel[j0].m_y, m_colorChannel[j0].m_z, m_colorChannel[j0].m_w);
						const ndVector dp(colorI - colorJ);
						for (ndInt32 k = 0; k < 3; ++k)
						{
							test &= (fabs(dp[k]) <= tolerance);
						}
...
void ndMeshEffect::ndAttibuteFormat::CompactVertexData(const ndPointFormat& points, ndInt32* const indexList, ndFloat32 tol)
{
	ndAttibuteFormat tmpFormat(*this);
	Clear();
...
bool ndMeshEffect::Sanity() const
{
	...
	{
		ndEdge* const edge = &iter.GetNode()->GetInfo();
		ndAssert(edge->m_twin);
		ndAssert(edge->m_next);
		ndAssert(edge->m_prev);
		ndAssert(edge->m_twin->m_twin == edge);
		ndAssert(edge->m_next->m_incidentVertex == edge->m_twin->m_incidentVertex);
		ndAssert(edge->m_incidentVertex == edge->m_twin->m_next->m_incidentVertex);
	}
	#endif
	return true;
}
...
{
	ndInt32 count = ndInt32 (m_points.m_vertex.GetCount()) - m_constructionIndex;
	if (count > 3) 
	{
		ndInt32 indexList[256];

		ndAssert(count < ndInt32(sizeof(indexList) / sizeof(indexList[0])));
		ndPolyhedra polygon;
		ndPointFormat points;
		ndAttibuteFormat attibutes;
		...
		m_points.m_vertex.SetCount(m_constructionIndex);
		m_attrib.SetCount(GetPropertiesCount() - count);
...
void ndMeshEffect::EndBuild(bool fixTjoint)
{
	...
	ndInt32 triangCount = ndInt32 (m_points.m_vertex.GetCount() / 3);
	...
	for (ndInt32 i = 0; i < triangCount; ++i) 
	{
		...
		if (mag2 > ndFloat64(1.0e-12f)) 
		{
			...
			if (!edge) 
			{
				ndAssert(0);
				/*
				//ndAssert ((m_pointCount + 3) <= m_maxPointCount);
...
bool ndMeshEffect::EndFace()
{
	...
	for (bool hasVertexCollision = true; hasVertexCollision;)
	{
		...
		for (ndList<ndNode*>::ndNode* node = collisionFound.GetFirst(); node; node = node->GetNext())
		{
			...
			if (m_points.m_skinWeights.GetCount())
			{
				m_points.m_skinWeights.PushBack(m_points.m_skinWeights[edge->m_incidentVertex]);
			}
...
void ndMeshEffect::BuildFromIndexList(const ndMeshVertexFormat* const format)
{
	...
	while (pendingFaces)
	{
		...
		for (ndInt32 j = 0; j < format->m_faceCount; ++j)
		{
			...
			if (indexCount > 0)
			{
				...
				ndAssert(indexCount >= 3);
				ndAssert(indexCount < ndInt32(sizeof(index) / sizeof(index[0])));

				if (!faceMark[j])
				{
					for (ndInt32 i = 0; i < indexCount; ++i)
					{
						ndInt32 k = m_attrib.GetCount() + i;
						userdata[i] = k;
						index[i] = vertexIndex[acc + i] + vertexBank;
					}

					const ndBigVector p0(m_points.m_vertex[index[0]]);
					const ndBigVector p1(m_points.m_vertex[index[1]]);
					ndBigVector e1(p1 - p0);
					ndBigVector normal(ndBigVector::m_zero);
					for (ndInt32 i = 2; i < indexCount; ++i)
					{
						const ndBigVector p2(m_points.m_vertex[index[i]]);
						const ndBigVector e2(p2 - p0);
						normal += e1.CrossProduct(e2);
						e1 = e2;
					}

					ndAssert(normal.m_w == ndFloat32(0.0f));
					ndFloat64 area = normal.DotProduct(normal).GetScalar();
					if (area < ndFloat32(1.0e-12f))
					{
						faceMark[j] = 1;
						acc += indexCount;
					...
					bool singleSize = true;
#if 1
					ndInt32 k0 = indexCount - 1;
					normal = normal.Normalize();
					for (ndInt32 k1 = 0; k1 < indexCount; ++k1)
					{
						const ndEdge* const twinEdge = FindEdge(index[k1], index[k0]);
						if (twinEdge && (twinEdge->m_incidentFace > 0))
						{
							ndBigVector twinNormal(FaceNormal(twinEdge, &m_points.m_vertex[0][0], format->m_vertex.m_strideInBytes));
							ndAssert(twinNormal.DotProduct(twinNormal).GetScalar() > ndFloat32(1.0e-20f));

							twinNormal = twinNormal.Normalize();
							ndFloat64 doubleFace = twinNormal.DotProduct(normal).GetScalar();
							if (doubleFace < ndFloat32(-0.999f))
							{
								singleSize = false;
								break;
							}
...
#endif
					// this make more robust but may not be nesserary
					ndEdge* const edge = singleSize ? AddFace(indexCount, index, userdata) : nullptr;
		...
		if (pendingFaces)
		{
			...
			{
				m_points.m_layers.PushBack(layerIndex);
				ndBigVector p(m_points.m_vertex[i]);
				m_points.m_vertex.PushBack(p);
			}
			if (format->m_vertexWeight.m_data)
			{
	...
	//ndAssert(m_attrib.m_pointChannel.GetCount() == attributeCount);
	ndAssert(m_points.m_vertex.GetCount() == vertexCount * (layerIndex + 1));
	ndAssert(!format->m_vertexWeight.m_data || (m_points.m_skinWeights.GetCount() == vertexCount * (layerIndex + 1)));
...
{
	ndStack<ndInt32>vertexIndexMapBuffer(ndInt32 (m_points.m_vertex.GetCount()));
	...
	for (iter.Begin(); iter; iter++) 
	{
		...
		if ((edge->m_mark != mark) && (edge->m_incidentFace > 0)) 
		{
			...
			if (!face) 
			{
				ndTrace(("skiping degeneraded face\n"));
				//ndAssert (0);
			}
		}
	}
...
ndAssert(0);
	...
	ndAssert(m_points.m_vertex.GetCount() == m_attrib.m_pointChannel.GetCount());
	#ifdef _DEBUG
	for (ndInt32 i = 0; i < m_attrib.m_pointChannel.GetCount(); ++i)
	{
		ndAssert(m_attrib.m_pointChannel[i] == i);
	}
	#endif
#endif
}
...
void ndMeshEffect::PackAttibuteData()
{
	...
	for (iter.Begin(); iter; iter++)
	{
		...
		if (edge->m_incidentFace > 0)
		{
			edge->m_userData = ndUnsigned64(attrIndexMap[ndInt32(edge->m_userData)]);
		}
	}

	ndMemSet(&attrIndexMap[0], -1, ndInt32(m_attrib.m_pointChannel.GetCount()));
	ndAttibuteFormat tmpFormat(m_attrib);
	m_attrib.Clear();
	...
	for (iter.Begin(); iter; iter++)
	{
		...
		if (edge->m_incidentFace > 0)
		{
			...
			if (attrIndexMap[ndInt32 (edge->m_userData)] == -1)
			{
...
{
	ndAttibuteFormat attibutes(m_attrib);
	m_attrib.Clear();
...
return 0;
	...
	ndAssert(p2p0.m_w == ndFloat32(0.0f));
	...
	ndAssert(t1 >= ndFloat64(0.0f));
	ndAssert(t1 <= ndFloat64(1.0f));
...
void ndMeshEffect::RepairTJoints()
{
	...
	while (dirty)
	{
		...
		for (iter.Begin(); iter; )
		{
			...
			if (mag2 < tol2)
			{
				...
				{
					//ndAssert (0);
					ndTrace(("remember to finish this!!: %s %s line:%d\n", __FILE__, __FUNCTION__, __LINE__));
					dirty = true;
	...
	while (dirty)
	{
		...
		for (iter1.Begin(); iter1; )
		{
			...
			ndBigVector A(p1 - p0);
			ndBigVector B(p2 - p1);
			ndAssert(A.m_w == ndFloat32(0.0f));
			ndAssert(B.m_w == ndFloat32(0.0f));
			...
			if (ab >= 0.0f)
			{
				...
				if (magab2 >= magaabb)
				{
					...
					{
						ndEdge* const openEdge = edge;
						//ndAssert (openEdge->m_incidentFace <= 0);
						ndTrace(("remember to finish this!!: %s %s line:%d\n", __FILE__, __FUNCTION__, __LINE__));
						ndEdge* const nextEdge = openEdge->m_next;
						ndEdge* const deletedEdge = openEdge->m_prev;
						if (deletedEdge == openEdge->m_next->m_next)
						{
							...
							//ndAssert (deletedEdge->m_twin->m_incidentFace > 0);
							ndTrace(("remember to finish this!!: %s %s line:%d\n", __FILE__, __FUNCTION__, __LINE__));
							openEdge->m_incidentFace = deletedEdge->m_twin->m_incidentFace;
							openEdge->m_next->m_incidentFace = deletedEdge->m_twin->m_incidentFace;
					...
					else
					{
						...
						ndAssert(A0.m_w == ndFloat32(0.0f));
						ndAssert(B0.m_w == ndFloat32(0.0f));
						...
						if (ab0 >= ndFloat32(0.0f))
						{
							...
							if (ab0ab0 >= aa0bb0)
							{
								if (openEdge->m_next->m_next->m_next->m_next != openEdge)
								{
									...
									ndAssert(A1.m_w == ndFloat32(0.0f));
									ndAssert(B1.m_w == ndFloat32(0.0f));
...
ndInt32 ndMeshEffect::GenerateVertexFormat(ndMeshVertexFormat& format, ndArray<ndUnsigned8>& buffer) const
{
	...
	if (m_attrib.m_normalChannel.m_isValid)
	{
		normalIndexOffset = totalSize;
		totalSize += indexCount * sizeof(ndInt32);
	}

	ndInt32 uvIndexOffset = 0;
	if (m_attrib.m_uv0Channel.m_isValid)
	{
		uvIndexOffset = totalSize;
		totalSize += indexCount * sizeof(ndInt32);
	}
	...
	if (m_attrib.m_normalChannel.m_isValid)
	{
		...
		for (ndInt32 i = 0; i < m_attrib.m_normalChannel.GetCount(); ++i)
		{
			format.m_normal.m_data[i * 3 + 0] = m_attrib.m_normalChannel[i].m_x;
			format.m_normal.m_data[i * 3 + 1] = m_attrib.m_normalChannel[i].m_y;
			format.m_normal.m_data[i * 3 + 2] = m_attrib.m_normalChannel[i].m_z;
		}
	}
	...
	if (m_attrib.m_uv0Channel.m_isValid)
	{
		...
		for (ndInt32 i = 0; i < m_attrib.m_uv0Channel.GetCount(); ++i)
		{
			format.m_uv0.m_data[i * 2 + 0] = m_attrib.m_uv0Channel[i].m_u;
			format.m_uv0.m_data[i * 2 + 1] = m_attrib.m_uv0Channel[i].m_v;
		}
	}
		...
		if ((edge->m_incidentFace > 0) && (edge->m_mark != lru))
		{
			...
			do
			{
				...
				if (m_attrib.m_normalChannel.m_isValid)
				{
					...
					format.m_normal.m_indexList[indexCount] = ndInt32(ptr->m_userData);
				}
				...
				{
					//m_attrib.m_uv0Channel.PushBack(attibutes.m_uv0Channel[ndInt32(ptr->m_userData)]);
					format.m_uv0.m_indexList[indexCount] = ndInt32(ptr->m_userData);
				}
	...
	if (m_points.m_skinWeights.m_isValid)
	{
		...
		{
			ndFloat64 m_posit[3];
			ndFloat64 m_boneIndex[ND_VERTEX_WEIGHT_SIZE];
			ndFloat64 m_boneWeight[ND_VERTEX_WEIGHT_SIZE];
		};
		...
		for (ndInt32 i = 0; i < indexCount; i++)
		{
			...
			{
				data.m_boneIndex[j] = format.m_vertexWeight.m_data[i].m_boneId[j];
				data.m_boneWeight[j] = format.m_vertexWeight.m_data[i].m_weight[j];
			}
			points.PushBack(data);
		}
		ndInt32 count = ndVertexListToIndexList(&points[0].m_posit[0], sizeof(ndVertexPositWeight), sizeof(ndVertexPositWeight)/sizeof (ndFloat64), indexCount, &tempIndex[0]);

		//#ifdef _DEBUG
		#if 0
		for (ndInt32 i = 0; i < indexCount; i++)
		{
			ndInt32 j0 = format.m_vertex.m_indexList[i];
			ndBigVector xxx0(format.m_vertex.m_data[j0 * 3 + 0], format.m_vertex.m_data[j0 * 3 + 1], format.m_vertex.m_data[j0 * 3 + 2], 0.0);

			ndInt32 j1 = tempIndex[format.m_vertex.m_indexList[i]];
			ndBigVector xxx1(points[j1].m_posit[0], points[j1].m_posit[1], points[j1].m_posit[2], 0.0);
			ndAssert(xxx0.m_x == xxx1.m_x);
			ndAssert(xxx0.m_y == xxx1.m_y);
			ndAssert(xxx0.m_z == xxx1.m_z);

			for (ndInt32 j = 0; j < ND_VERTEX_WEIGHT_SIZE; ++j)
			{
				ndInt32 k0 = format.m_vertexWeight.m_data[j0].m_boneId[j];
				ndInt32 k1 = ndInt32(points[j1].m_boneIndex[j]);
				ndAssert(k0 == k1);

				ndReal w0 = format.m_vertexWeight.m_data[j0].m_weight[j];
				ndReal w1 = ndReal(points[j1].m_boneWeight[j]);
				ndAssert(w0 == w1);
			}
		}
		#endif
		...
		for (ndInt32 i = 0; i < count; i++)
		{
			...
			for (ndInt32 j = 0; j < ND_VERTEX_WEIGHT_SIZE; ++j)
			{
				format.m_vertexWeight.m_data[i].m_boneId[j] = ndInt32 (points[i].m_boneIndex[j]);
				format.m_vertexWeight.m_data[i].m_weight[j] = ndReal (points[i].m_boneWeight[j]);
			}
		}
		for (ndInt32 i = 0; i < indexCount; i++)
		{
			ndInt32 index = tempIndex[format.m_vertex.m_indexList[i]];
			format.m_vertex.m_indexList[i] = index;
			format.m_vertexWeight.m_indexList[i] = index;
		}
	}
	else
	{
		...
		if ((face->m_incidentFace > 0) && (face->m_mark < mark)) 
		{
			...
			do {
				...
				if (source->m_attrib.m_materialChannel.GetCount()) 
				{
					...
					maxMatIndex = ndMax(matIndex, maxMatIndex);
	...
	,m_constructionIndex(0)
{
	...
	if (count >= 4) 
	{
		...
		if (convexHull.GetCount()) 
		{
			...
			format.m_vertex.m_data = (ndFloat64*)&convexHull.GetVertexPool()[0].m_x;
...
void ndMeshEffect::ConvertToPolygons()
{
	...
	ndAssert(leftOversOut.GetCount() == 0);
...
void ndMeshEffect::SerializeToXml(nd::TiXmlElement* const xmlNode, const ndTree<ndString, ndUnsigned32>& boneMap) const
{
	ndArray<ndInt32> faceIndexArray;
	ndArray<ndInt32> faceMaterialArray;

	ndArray<ndInt32> pointsIndexList;
	ndArray<ndInt32> normalsIndexList;
	ndArray<ndInt32> uvIndexList;
	...
	{
		// save mesh vertex data
		ndArray<ndTriplexReal> points;
		for (ndInt32 i = 0; i < ndInt32(m_points.m_vertex.GetCount()); ++i)
		{
			ndTriplexReal p;
			ndReal x = ndReal(m_points.m_vertex[i].m_x);
			ndReal y = ndReal(m_points.m_vertex[i].m_y);
			ndReal z = ndReal(m_points.m_vertex[i].m_z);
			p.m_x = x;
			p.m_y = y;
			p.m_z = z;
			points.PushBack(p);
		...
		nd::TiXmlElement* const xmlVertexNode = new nd::TiXmlElement("vertices");
		xmlNode->LinkEndChild(xmlVertexNode);

		ndArray<ndInt32> indexArray;
		ndArray<ndInt32> remapIndexList;
		for (ndInt32 i = 0; i < ndInt32(pointsIndexList.GetCount()); ++i)
		{
			remapIndexList.PushBack(pointsIndexList[i]);
		}
		indexArray.SetCount(ndInt32(points.GetCount()));
		pointCount = ndVertexListToIndexList(&points[0].m_x, sizeof(ndTriplexReal), 3, ndInt32(points.GetCount()), &indexArray[0], ndFloat32(1.0e-8f));
		points.SetCount(pointCount);
		for (ndInt32 i = 0; i < ndInt32(pointsIndexList.GetCount()); ++i)
		{
			pointsIndexList[i] = indexArray[remapIndexList[i]];
		}
		xmlSaveParam(xmlVertexNode, "positions", points);
		xmlSaveParam(xmlVertexNode, "indices", pointsIndexList);

		if (m_points.m_skinWeights.GetCount())
		{
			ndArray<ndVertexWeight> weightArray;
			weightArray.SetCount(pointCount);
			for (ndInt32 i = 0; i < ndInt32(m_points.m_skinWeights.GetCount()); ++i)
			{
				ndInt32 index = indexArray[i];
				weightArray[index] = m_points.m_skinWeights[i];
			}
			...
			{
				const ndVertexWeight& weight = weightArray[i];
				ndAssert(weight.m_boneId[0] != -1);

				nd::TiXmlElement* const xmlVertexWeight = new nd::TiXmlElement("vert");
				xmlVertexWeights->LinkEndChild(xmlVertexWeight);
				xmlSaveAttribute(xmlVertexWeight, "vertID", i);

				for (ndInt32 j = 0; j < ND_VERTEX_WEIGHT_SIZE; ++j)
				{
					if (weight.m_boneId[j] != -1)
					{
						const ndTree<ndString, ndUnsigned32>::ndNode* const nameNode = boneMap.Find(ndUnsigned32(weight.m_boneId[j]));
						ndAssert(nameNode);
						xmlSaveAttribute(xmlVertexWeight, boneNames[j], nameNode->GetInfo().GetStr());
						xmlSaveAttribute(xmlVertexWeight, weightNames[j], weight.m_weight[j]);
					}
				}
			}
		}
...

	{
		...
		ndArray<ndInt32> indexArray;
		ndArray<ndInt32> remapIndexList;
		for (ndInt32 i = 0; i < ndInt32(normalsIndexList.GetCount()); ++i)
		{
			remapIndexList.PushBack(normalsIndexList[i]);
		}
		indexArray.SetCount(ndInt32(points.GetCount()));
		ndInt32 vertexCount = ndVertexListToIndexList(&points[0].m_x, sizeof(ndTriplexReal), 3, ndInt32(points.GetCount()), &indexArray[0], ndFloat32(1.0e-8f));
		points.SetCount(vertexCount);
		for (ndInt32 i = 0; i < ndInt32(normalsIndexList.GetCount()); ++i)
		{
			normalsIndexList[i] = indexArray[remapIndexList[i]];
		}
		xmlSaveParam(node, "normals", points);
		xmlSaveParam(node, "indices", normalsIndexList);
	}

	{
		...
		ndArray<ndInt32> indexArray;
		ndArray<ndInt32> remapIndexList;
		for (ndInt32 i = 0; i < ndInt32(uvIndexList.GetCount()); ++i)
		{
			remapIndexList.PushBack(uvIndexList[i]);
		}
		indexArray.SetCount(ndInt32(points.GetCount()));
		ndInt32 vertexCount = ndVertexListToIndexList(&points[0].m_x, sizeof(ndTriplexReal), 3, ndInt32(points.GetCount()), &indexArray[0], ndFloat32(1.0e-8f));
		points.SetCount(vertexCount);
		for (ndInt32 i = 0; i < ndInt32(uvIndexList.GetCount()); ++i)
		{
			uvIndexList[i] = indexArray[remapIndexList[i]];
		}
		xmlSaveParam(node, "uv", points);
		xmlSaveParam(node, "indices", uvIndexList);
	}

	const ndArray<ndMaterial>& materials = GetMaterials();
...

	{
		...
		while (duplicateCount)
		{
			duplicateCount = 0;
			ndInt32 start = 0;
			for (ndInt32 i = 0; i < ndInt32(faceIndexArray.GetCount()); ++i)
			{
				if (!faceMask[i])
				{
					ndInt32 face[256];
					const ndInt32 count = faceIndexArray[i];
					for (ndInt32 j = 0; j < count; ++j)
					{
						face[j] = pointsIndexList[start + j] + base;
					}
					ndEdge* const edge = simplex.AddFace(count, face);
					if (edge)
					{
						faceMask[i] = 1;
						layers[i] = layerIndex;
		...
		if (layerIndex)
		{
			xmlSaveParam(node, "faceLayers", layers);
		}
		xmlSaveParam(node, "faceMaterial", faceMaterialArray);
		xmlSaveParam(node, "faceIndexCount", faceIndexArray);
	}
}
...
void ndMeshEffect::DeserializeFromXml(const nd::TiXmlElement* const xmlNode)
{
	...
	ndArray<ndVertexWeight> vertexWeights;
	ndArray<ndInt32> vertexIndexArray;
	{
		...
		if (xmlVertexWeightsNode)
		{
			...
			for (const nd::TiXmlNode* node = xmlVertexWeightsNode->FirstChild("vert"); node; node = node->NextSibling("vert"))
			{
				...
				ndVertexWeight weight;
				for (ndInt32 i = 0; i < 4; ++i)
				{
					const char* const boneName = xmlGetNameAttribute(linkNode, boneNames[i]);
					if (boneName)
					{
				...
				vertexWeights[vertIndex] = weight;
			}
			format.m_vertexWeight.m_data = &vertexWeights[0];
			format.m_vertexWeight.m_indexList = &vertexIndexArray[0];
	...
	if (xmlNode->FirstChild("normal"))
	{
		...
		xmlGetFloatArray3(node, "normals", normalArray);
