...
void ndJointBallAndSocket::EnableCone(bool state)
{
	m_options.m_option4 = state;
}

void ndJointBallAndSocket::SetTwistLimits(dFloat minAngle, dFloat maxAngle)
{
	m_minTwistAngle = -dAbs (minAngle);
	m_maxTwistAngle = dAbs(maxAngle);
}

void ndJointBallAndSocket::GetTwistLimits(dFloat& minAngle, dFloat& maxAngle) const
{
	minAngle = m_minTwistAngle;
	maxAngle = m_maxTwistAngle;
}

void ndJointBallAndSocket::SetTwistFriction (dFloat frictionTorque)
{
	m_twistFriction = dAbs (frictionTorque);
}

dFloat ndJointBallAndSocket::GetTwistFriction (dFloat frictionTorque) const
{
	return m_twistFriction;
}

dFloat ndJointBallAndSocket::GetConeLimits() const
{
	return m_maxConeAngle;
}

void ndJointBallAndSocket::SetConeLimits(dFloat maxAngle)
{
	m_maxConeAngle = dMin (dAbs (maxAngle), D_BALL_AND_SOCKED_MAX_CONE_ANGLE);
}

void ndJointBallAndSocket::SetConeFriction(dFloat frictionTorque)
{
	m_coneFriction = dAbs (frictionTorque);
}

dFloat ndJointBallAndSocket::GetConeFriction(dFloat frictionTorque) const
{
	return m_coneFriction;
}

void ndJointBallAndSocket::Debug(dDebugDisplay* const debugDisplay) const
{
	dMatrix matrix0;
	dMatrix matrix1;
	...
	if (m_options.m_option3 && ((m_maxTwistAngle - m_minTwistAngle) > dFloat(1.0e-3f))) {
...
void ndJointBallAndSocket::SubmitTwistAngle(const dVector& pin, dFloat angle, dFloat timestep)
{
...
void ndJointBallAndSocket::SubmitAngularAxisCartisianApproximation(const dMatrix& matrix0, const dMatrix& matrix1, dFloat timestep)
{
	if (m_options.m_option4) {
...
void ndJointBallAndSocket::SubmitAngularAxis(const dMatrix& matrix0, const dMatrix& matrix1, dFloat timestep)
{
