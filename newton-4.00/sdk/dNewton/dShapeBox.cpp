...
dShapeBox::dShapeBox(dgMemoryAllocator* allocator, dgUnsigned32 signature, dFloat32 size_x, dFloat32 size_y, dFloat32 size_z)
	:dShapeConvex(allocator, signature, m_boxCollision)
{
	Init (size_x, size_y, size_z);
}

dShapeBox::dShapeBox(dgWorld* const world, dgDeserialize deserialization, void* const userData, dInt32 revisionNumber)
	:dShapeConvex (world, deserialization, userData, revisionNumber)
{
	dVector size;
...
void dShapeBox::SetCollisionBBox (const dVector& p0__, const dVector& p1__)
{
	dAssert (0);
}

dInt32 dShapeBox::CalculateSignature (dFloat32 dx, dFloat32 dy, dFloat32 dz)
{
	...
	buffer[1] = Quantize (dx * dFloat32 (0.5f));
	buffer[2] = Quantize (dy * dFloat32 (0.5f));
	buffer[3] = Quantize (dz * dFloat32 (0.5f));
	return Quantize(buffer, sizeof (buffer));
}

dInt32 dShapeBox::CalculateSignature () const
{
...
dVector dShapeBox::SupportVertexSpecial(const dVector& dir0, dFloat32 skinThickness, dInt32* const vertexIndex) const
{
	...
	dAssert(dAbs(dir.DotProduct(dir).GetScalar() - dFloat32(1.0f)) < dFloat32(1.0e-3f));
	dAssert(dir.m_w == dFloat32(0.0f));
//	dVector mask(dir < dVector(dFloat32(0.0f)));
	dVector mask(dir < dVector::m_zero);
	if (vertexIndex) {
		dVector index(m_indexMark * (mask & dVector::m_one));
		index = (index.AddHorizontal()).GetInt();
	...
	//dVector padd (DG_PENETRATION_TOL);
	//padd = padd & dVector::m_triplexMask;
	dVector size0 (m_size[0] - m_penetrationTol);
	dVector size1 (m_size[1] + m_penetrationTol);
	return size0.Select(size1, mask);
}

dVector dShapeBox::SupportVertexSpecialProjectPoint(const dVector& point, const dVector& dir0) const
{
	dVector mask0(dir0.Abs() > m_flushZero);
	dVector dir(dir0 & mask0);
	dAssert(dAbs((dir.DotProduct(dir).GetScalar() - dFloat32(1.0f))) < dFloat32(1.0e-3f));
...
{
	dShapeConvex::GetCollisionInfo(info);
...
void dShapeBox::Serialize(dgSerialize callback, void* const userData) const
{
	SerializeLow(callback, userData);
	dVector size (m_size[0].Scale (dFloat32 (2.0f)));
