...
ndRandom::ndRandom(ndUnsigned32 seed)
	:m_generator()
{
	if (seed)
	{
...
ndFloat32 ndRandom::GetRand()
{
	...
	ndFloat32 r = ndFloat32(GetRandInt()) / (ndFloat32)spand;
	return r;
}
...
ndFloat32 ndRandom::GetGaussianRandom(ndFloat32 mean, ndFloat32 sigma)
{
	...
	auto NormalCumulativeDistibutionInverse = [](ndFloat32 r)
	{
		...
		// Taylor expansion does not produce good approximation, for value far from the center 
	...
	ndFloat32 r = ndClamp(GetRand(), ndFloat32(1.0e-6f), ndFloat32(1.0f - 1.0e-6f));
...
static ndRandom& GetRandomGenerator(ndUnsigned32 seed = 0)
{
	static ndRandom generator (seed);
	if (seed)
	{
		ndScopeSpinLock lock(GetLock());
		generator = ndRandom(seed);
	}
	return generator;
}
...
{
	ndRandom& generator = GetRandomGenerator();
	ndScopeSpinLock lock(GetLock());
	return generator.GetRandInt();
}
