...
void ndIsoSurface::ndImplementation::ProcessLowResCell(ndIsoCell& cell)
{
	...
	int scan[256];
	int faces[256];
	ndArray<ndEdge> edge____;
	
	for (int xxx = 0; xxx < 256; xxx++)
	{
		const int* xxxxxx = &m_triangleTable[xxx][0];
	
		int faceCount = 0;
		for (ndInt32 j = 0; xxxxxx[j] != -1; j += 3)
		{
			faceCount ++;
		}
		faces[xxx] = faceCount;
		...
		ndEdge edge;
		ndInt32 edgeBits = m_edgeTable[xxx];
		if (edgeBits & 1)
		{
	...
	dTrace(("ndEdge m_edges[] =\n{\n"));
	for (int xxx = 0; xxx < edge____.GetCount(); xxx++)
	{
		dTrace(("\t{%d, %d, %d},\n", edge____[xxx].m_midPoint, edge____[xxx].m_p0, edge____[xxx].m_p1));
	}
	dTrace(("};\n"));

	dTrace(("ndInt32 m_faces[][3] =\n{\n"));
	for (int xxx = 0; xxx < 256; xxx++)
	{
		const ndInt32* ptr = &m_triangleTable[xxx][0];
		for (ndInt32 i = 0; ptr[i] != -1; i += 3)
		{
			const ndInt32 j0 = ptr[i + 0];
			const ndInt32 j1 = ptr[i + 1];
			const ndInt32 j2 = ptr[i + 2];
			dTrace(("\t{%d, %d, %d},\n", j0, j1, j2));
		}
	}
	dTrace(("};\n"));
