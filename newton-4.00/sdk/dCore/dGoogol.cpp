...
dGoogol dGoogol::operator+ (const dGoogol &A) const
{
	...
	{
		dUnsigned64 mantissa0[DG_GOOGOL_SIZE];
		dUnsigned64 mantissa1[DG_GOOGOL_SIZE];
		dUnsigned64 mantissa[DG_GOOGOL_SIZE];
		...
		for (dInt32 i = DG_GOOGOL_SIZE - 1; i >= 0; i --) 
		{
...
dGoogol dGoogol::operator* (const dGoogol &A) const
{
	...
	if (m_mantissa[0] && A.m_mantissa[0]) 
	{
		dUnsigned64 mantissaAcc[DG_GOOGOL_SIZE * 2];
		...
		for (dInt32 i = DG_GOOGOL_SIZE - 1; i >= 0; i --) 
		{
			...
			if (a) 
			{
				dUnsigned64 mantissaScale[2 * DG_GOOGOL_SIZE];
				memset (mantissaScale, 0, sizeof (mantissaScale));
				A.ScaleMantissa (&mantissaScale[i], a);

				dUnsigned64 carrier = 0;
				for (dInt32 j = 0; j < 2 * DG_GOOGOL_SIZE; j ++) 
				{
					const dInt32 k = 2 * DG_GOOGOL_SIZE - 1 - j;
		...
		for (dInt32 i = 0; i < 2 * DG_GOOGOL_SIZE; i ++) 
		{
			const dInt32 k = 2 * DG_GOOGOL_SIZE - 1 - i;
...
dGoogol dGoogol::operator/ (const dGoogol &A) const
{
	...
	dAssert (passes <= (2 * DG_GOOGOL_SIZE));
	return (*this) * tmp;
}
...
dGoogol dGoogol::Floor () const
{
	...
	dGoogol tmp (*this);
	for (dInt32 i = DG_GOOGOL_SIZE - 1; i > start; i --) 
	{
		tmp.m_mantissa[i] = 0;
	}
...
void dGoogol::ShiftRightMantissa (dUnsigned64* const mantissa, dInt32 bits) const
{
	...
	while (bits >= 64) 
	{
		for (dInt32 i = DG_GOOGOL_SIZE - 2; i >= 0; i --) 
		{
	...
	{
		carrier <<= (64 - bits);
		for (dInt32 i = 0; i < DG_GOOGOL_SIZE; i ++) 
		{
...
dInt32 dGoogol::NormalizeMantissa (dUnsigned64* const mantissa) const
{
	...
	else 
	{
		...
		{
			bits -= 64;
			for (dInt32 i = 1; i < DG_GOOGOL_SIZE; i ++) {
				mantissa[i - 1] = mantissa[i];
			}
