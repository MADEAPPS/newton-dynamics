...
void ndAabbPolygonSoup::CalculateAdjacendy ()
{
	...
	ForAllSectors(box, ndVector::m_zero, ndFloat32(1.0f), CalculateAllFaceEdgeNormals, &adjacenyMesh);
	adjacenyMesh.EndFace();

	ndInt32 mark = adjacenyMesh.IncLRU();
				...
				if (dot > 0.0f) 
				{
					...
					if ((dot * dot) >= (q1p0Mag2 * q1q0Mag2 * ndFloat64(0.99995f))) 
					{
						...
						if (!((y0 >= x1) || (y1 <= x0))) 
						{
							...
							dAssert (q1q0.m_w == ndFloat32 (0.0f));
...
void ndAabbPolygonSoup::ForAllSectors (const ndFastAabb& obbAabbInfo, const ndVector& boxDistanceTravel, ndFloat32, dAaabbIntersectCallback callback, void* const context) const
{
	...
	if (m_aabb) 
	{
		...
		dAssert (boxDistanceTravel.m_w == ndFloat32 (0.0f));
