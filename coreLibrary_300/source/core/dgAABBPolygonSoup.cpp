...
class dgAABBTree
{
	...
	void ForAllSectors (const dgInt32* const indexArray, const dgFloat32* const vertexArray, const dgVector& minBox, const dgVector& maxBox, const dgVector& boxDistanceTravel, dgFloat32 m_maxT, dgAABBIntersectCallback callback, void* const context) const
	{
		...
		if ((boxDistanceTravel % boxDistanceTravel) < dgFloat32 (1.0e-8f)) {
			stackPool[0] = this;
			distance[0] = BoxPenetration ((dgTriplex*)vertexArray, minBox, maxBox);
...
//				if (me->BoxTest ((dgTriplex*) vertexArray, minBox, maxBox)) {
				if (dist > dgFloat32 (0.0f)) {
					if (me->m_back.IsLeaf()) {
						...
						dgFloat32 dist = node->BoxPenetration ((dgTriplex*)vertexArray, minBox, maxBox);
						if (dist > dgFloat32 (0.0f)) {
...
{
	public:
	dgNodeBuilder (dgTriplex* const vertexArray, dgInt32 indexCount, const dgInt32* const indexArray)
		:dgNode()
		,m_parent (NULL)
		,m_enumeration(0)
		...
		,m_faceIndices(indexArray)
	{
		...
		vertexArray[m_indexBox0].m_x = minP.m_x;
		vertexArray[m_indexBox0].m_y = minP.m_y;
		vertexArray[m_indexBox0].m_z = minP.m_z;
		vertexArray[m_indexBox1].m_x = maxP.m_x;
		vertexArray[m_indexBox1].m_y = maxP.m_y;
		vertexArray[m_indexBox1].m_z = maxP.m_z;
...
void dgAABBPolygonSoup::Create (dgMemoryAllocator* const allocator, const dgPolygonSoupDatabaseBuilder& builder, bool optimizedBuild)
{
	...
	m_indexCount = builder.m_indexCount * 2 + builder.m_faceCount;
	m_indices = (dgInt32*) dgMallocStack (sizeof (dgInt32) * m_indexCount);
	m_aabb = (dgNode*) dgMallocStack (sizeof (dgNode) * (builder.m_faceCount * 2 - 1));

//	m_localVertex = (dgFloat32*) dgMallocStack (sizeof (dgTriplex) * (builder.m_vertexCount + builder.m_normalCount + builder.m_faceCount * 4));
	m_localVertex = (dgFloat32*) dgMallocStack (sizeof (dgTriplex) * (builder.m_vertexCount + builder.m_normalCount));
	dgTriplex* const tmpVertexArray = (dgTriplex*) m_localVertex;
...
//	dgInt32* indexMap = m_indices;
	const dgInt32* const indices = &builder.m_vertexIndex[0];
	dgNodeBuilder* const constructor = (dgNodeBuilder*) dgMallocStack (sizeof (dgNodeBuilder) * (builder.m_faceCount * 2 + 1)); 
	...
	for (dgInt32 i = 0; i < builder.m_faceCount; i ++) {
		dgInt32 indexCount = builder.m_faceVertexCount[i] - 1;
		//dgInt32 boxIndex = builder.m_normalCount + builder.m_vertexCount + i * 2;
		dgNodeBuilder* const newNode = new (&constructor[allocatorIndex]) dgNodeBuilder (tmpVertexArray, indexCount, &indices[polygonIndex]);
		allocatorIndex ++;
		if (!root) {
			root = newNode;
		} else {
			...
			if (!sibling->m_parent) {
				...
				allocatorIndex ++;
				dgAssert (allocatorIndex <= (builder.m_faceCount * 2 - 1));
			} else {
...
//		dgInt32 maxPasses = 2 * dgExp2 (m_array.GetCount() * 2) + 1;
	...
	dgStack<dgVector> aabbMem (builder.m_faceCount * 2);
...
/*
		...
		aabbNode.m_left = node->m_left ? &m_aabb[((dgNodeBuilder*)node->m_left)->m_enumeration] : NULL;
		aabbNode.m_right = node->m_right ? &m_aabb[((dgNodeBuilder*)node->m_right)->m_enumeration] : NULL;

		aabbPoints[vertexIndex] = node->m_p0;
		tmpVertexArray[vertexIndex + 1].m_x = node->m_p1;
