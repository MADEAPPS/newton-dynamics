...
//					}
	...
	dgInt32 ReplaceFaceNodes (dgListNode* const faceNode, dgInt32 faceIndex, const dgBigVector& vertex)
	{
		...
		dgAssert (SanityPointInTetra (tetra, vertex));
		dgAssert (SanityPointInTetra (neighborghTetra, vertex));
			...
			if (!m_edgeMap.Find(key)) {
				...
				for (dgEdgeSharedTetras::dgListNode* node = tetraMap.GetFirst(); node; node = node->GetNext()) {
					...
					if (baricentric.m_w == dgFloat64 (0.0f)) {
						dgAssert ((baricentric.m_x > dgFloat64 (0.0f)) && (baricentric.m_y > dgFloat64 (0.0f)) && (baricentric.m_z > dgFloat64 (0.0f)));
...
*/
		...
		for (dgEdgeSharedTetras::dgListNode* ptr = header.GetFirst(); ptr; ptr = ptr->GetNext()) {
			...
			for (dgInt32 i = 0; i < 4; i ++) {
				...
				if (((i0 == k0) && (i1 == k1)) || ((i1 == k0) && (i2 == k1)) || ((i2 == k0) && (i0 == k1))) {
					...
					dgAssert (index[0] == faceEdge->m_incidentVertex);
					dgAssert (index[1] == faceEdge->m_next->m_incidentVertex);
					...
					if (nextEdge != faceEdge) {
						if (nextEdge->m_prev != faceEdge->m_next) {
							dgEdge* const edge = mesh.ConnectVertex(faceEdge->m_next, nextEdge);
							dgAssert (edge);
							dgAssert (edge->m_next);
							dgAssert (edge->m_prev);
							dgAssert (edge->m_twin->m_next);
							dgAssert (edge->m_twin->m_prev);
							dgAssert (faceEdge->m_next == edge->m_twin);
						}
						if (nextEdge->m_next != faceEdge) {
#ifdef	_DEBUG
							dgEdge* const edge = mesh.ConnectVertex(faceEdge, nextEdge);
							dgAssert (edge);
							dgAssert (edge->m_next);
							dgAssert (edge->m_prev);
							dgAssert (edge->m_twin->m_next);
							dgAssert (edge->m_twin->m_prev);
							dgAssert (faceEdge->m_prev == edge);
#else
							mesh.ConnectVertex(faceEdge, nextEdge);
#endif
	...
	void RecoverFace (dgMeshEffect& mesh, dgEdge* const face, dgInt32 faceMark, dgInt32 tetraMark, dgTree<dgEdge*, dgEdge*>& edgeInconflict) const
	{
		...
		do {
			...
			count ++;
			dgAssert (count < dgInt32 (sizeof (edgeArray) / sizeof (edgeArray[0])));
			ptr = ptr->m_next;
		} while (ptr != face);
		...
		while (count) {
			...
			if (state) {
				dgAssert (triangleFace == triangleFace->m_next->m_next->m_next);
			...
			for (dgInt32 i = 1; i <= perimeterCount; i ++) {
				...
				for (dgEdge* edge = perimterEdges[i]->m_prev; edge != last; edge = edge->m_twin->m_prev) {
					if (edge->m_mark != faceMark) {
						...
						dgAssert (end);
						dgEdge* const newEdge = m_mesh->ConnectVertex(end, begin);
						dgAssert (!edgeInconflict.Find(newEdge));
...
/*
dgMeshEffect* dgMeshEffect::CreateVoronoiConvexDecomposition (dgInt32 pointCount, dgInt32 pointStrideInBytes, const dgFloat32* const pointCloud, dgInt32 materialId, const dgFloat32* const textureProjectionMatrix)
{
...
return NULL;
	...
	dgMeshEffect convexMesh (GetAllocator(), &convexPointCloud[0].m_x, count, sizeof (dgBigVector), dgFloat64 (0.0f));
	...
	if (convexMesh.GetCount()) {
		...
		intersection = Intersection (dgGetIdentityMatrix(), &convexMesh);
	}
...
dgMeshEffect* dgMeshEffect::CreateVoronoiConvexDecomposition (dgMemoryAllocator* const allocator, dgInt32 pointCount, dgInt32 pointStrideInBytes, const dgFloat32* const pointCloud, dgInt32 materialId, const dgMatrix& textureProjectionMatrix)
{
	...
	pool[count + 0] = dgBigVector ( pMin.m_x * dgFloat32 (2.0f), pMin.m_y * dgFloat32 (2.0f), pMin.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	pool[count + 1] = dgBigVector ( pMax.m_x * dgFloat32 (2.0f), pMin.m_y * dgFloat32 (2.0f), pMin.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	pool[count + 2] = dgBigVector ( pMin.m_x * dgFloat32 (2.0f), pMax.m_y * dgFloat32 (2.0f), pMin.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	pool[count + 3] = dgBigVector ( pMax.m_x * dgFloat32 (2.0f), pMax.m_y * dgFloat32 (2.0f), pMin.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	pool[count + 4] = dgBigVector ( pMin.m_x * dgFloat32 (2.0f), pMin.m_y * dgFloat32 (2.0f), pMax.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	pool[count + 5] = dgBigVector ( pMax.m_x * dgFloat32 (2.0f), pMin.m_y * dgFloat32 (2.0f), pMax.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	pool[count + 6] = dgBigVector ( pMin.m_x * dgFloat32 (2.0f), pMax.m_y * dgFloat32 (2.0f), pMax.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	pool[count + 7] = dgBigVector ( pMax.m_x * dgFloat32 (2.0f), pMax.m_y * dgFloat32 (2.0f), pMax.m_z * dgFloat32 (2.0f), dgFloat64 (0.0f));
	...
	dgDelaunayTetrahedralization delaunayTetrahedras (allocator, &pool[0].m_x, count, sizeof (dgBigVector), 0.0f);
