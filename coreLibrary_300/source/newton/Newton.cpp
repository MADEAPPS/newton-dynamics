...
void NewtonSetMinimumFrameRate(const NewtonWorld* const newtonWorld, dFloat frameRate)
{
	...
	TRACE_FUNCTION(__FUNCTION__);
	dgAssert (dgFloat32(1.0f) / dgFloat32 (DG_MAX_TIMESTEP) < dgFloat32(1.0f) / dgFloat32 (DG_MIN_TIMESTEP));
...
//	} 
//	return (NewtonCollision*) world->CreateEllipse (radiusX, radiusY, radiusZ, shapeID, matrix);
															   ...
															   const void* const elevationMap, const char* const attributeMap, dFloat verticalScale, dFloat horizontalScale, int shapeID)
{
	...
	TRACE_FUNCTION(__FUNCTION__);
	dgCollisionInstance* const collision = world->CreateHeightField(width, height, gridsDiagonals, elevationdatType, elevationMap, (const dgInt8* const) attributeMap, verticalScale, horizontalScale);
	collision->SetUserDataID(dgUnsigned32 (shapeID));
	return (NewtonCollision*) collision;
}
...
Hinge joint interfacefixme: all functions are commented out. Can we delete them?@{  fixme: delete NewtonConstraintCreateHinge?
  Create a hinge joint.

  @param *newtonWorld is the pointer to the Newton world.
  @param *pivotPoint is origin of the hinge in global space.
  @param *pinDir is the line of action of the hinge in global space.
  @param *childBody is the pointer to the attached rigid body, this body can not be NULL or it can not have an infinity (zero) mass.
  @param *parentBody is the pointer to the parent rigid body, this body can be NULL or any kind of rigid body.
...
NewtonJoint*  NewtonConstraintCreateHinge(const NewtonWorld* const newtonWorld, const dFloat* pivotPoint, const dFloat* pinDir, const NewtonBody* const childBody, const NewtonBody* const parentBody){TRACE_FUNCTION(__FUNCTION__);Newton* const world = (Newton *)newtonWorld;dgBody* const body0 = (dgBody *)childBody;dgBody* const body1 = (dgBody *)parentBody;dgVector pivot (pivotPoint[0], pivotPoint[1], pivotPoint[2], dgFloat32 (0.0f));dgVector pin (pinDir[0], pinDir[1], pinDir[2], dgFloat32 (0.0f));return (NewtonJoint*) world->CreateHingeConstraint (pivot, pin, body0, body1);}  fixme: delete NewtonHingeSetUserCallback?
...
void NewtonHingeSetUserCallback(const NewtonJoint* const hinge, NewtonHingeCallback callback){dgHingeConstraint* contraint;TRACE_FUNCTION(__FUNCTION__);contraint = (dgHingeConstraint*) hinge;contraint->SetJointParameterCallback ((dgHingeJointAcceleration)callback);}  fixme: delete NewtonHingeGetJointAngle?
  Get the relative joint angle between the two bodies.
  ...
  this function can be used during a function update call back to provide the application with some special effect.
  for example the application can play a bell sound when the joint angle passes some max value.
dFloat NewtonHingeGetJointAngle (const NewtonJoint* const hinge){dgHingeConstraint* contraint;TRACE_FUNCTION(__FUNCTION__);contraint = (dgHingeConstraint*) hinge;return contraint->GetJointAngle ();}  fixme: delete NewtonHingeGetJointOmega?
  Get the relative joint angular velocity between the two bodies.
  ...
  this function can be used during a function update call back to provide the application with some special effect.
  for example the application can play the creaky noise of a hanging lamp.
dFloat NewtonHingeGetJointOmega(const NewtonJoint* const hinge){dgHingeConstraint* contraint;TRACE_FUNCTION(__FUNCTION__);contraint = (dgHingeConstraint*) hinge;return contraint->GetJointOmega ();}  fixme: delete NewtonHingeGetJointForce?
  Calculate the angular acceleration needed to stop the hinge at the desired angle.

  @param *Hinge pointer to the joint.
  @param *desc is the pointer to and the Hinge or slide structure.
  @param angle is the desired hinge stop angle
...
dFloat NewtonHingeCalculateStopAlpha (const NewtonJoint* const hinge, const NewtonHingeSliderUpdateDesc* const desc, dFloat angle){dgHingeConstraint* contraint;TRACE_FUNCTION(__FUNCTION__);contraint = (dgHingeConstraint*) hinge;return contraint->CalculateStopAlpha (angle, (dgJointCallbackParam*) desc);}  fixme: delete NewtonHingeGetJointForce?
  Get the total force asserted over the joint pivot point, to maintain the constraint.
...
NewtonMesh* NewtonMeshCreateDelaunayTetrahedralization (const NewtonWorld* const newtonWorld, int pointCount, const dFloat* const vertexCloud, int strideInBytes, int materialID, const dFloat* const textureMatrix)
{
	TRACE_FUNCTION(__FUNCTION__);
	Newton* const world = (Newton *) newtonWorld;
...
void NewtonMeshGetVertexStreams(const NewtonMesh* const mesh, 
								dgInt32 vetexStrideInByte, dFloat* const vertex,
								dgInt32 normalStrideInByte, dFloat* const normal,
...
void NewtonMeshGetIndirectVertexStreams(const NewtonMesh* const mesh, 
										int vetexStrideInByte, dFloat* const vertex, int* const vertexIndices, int* const vertexCount,
