...
dgFloat32 dgCollisionHeightField::RayCastCellSimd (const dgFastRayTest& ray, dgInt32 xIndex0, dgInt32 zIndex0, dgVector& normalOut) const
{
	dgVector points[4];
	dgInt32 triangle[3];
...
dgFloat32 dgCollisionHeightField::RayCastSimd (const dgVector& q0, const dgVector& q1, dgContactPoint& contactOut, const dgBody* const body, void* const userData) const
{
	dgVector boxP0;
	dgVector boxP1;
		...
		do {
			...
			if (t < dgFloat32 (1.0f)) {
				...
				if (m_userRayCastCallback) {
					dgVector normal (body->GetCollision()->GetGlobalMatrix().RotateVectorSimd(contactOut.m_normal));
