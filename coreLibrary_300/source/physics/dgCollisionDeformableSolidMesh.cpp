...
void dgCollisionDeformableSolidMesh::InitRegions()
{
	...
	for (dgInt32 i = 0; i < m_regionsCount; i ++) {
		...
		m_regionAqqInv[i] = dgGetZeroMatrix();
	...
	memset (covarianceMatrix, 0, covarianceMatrixPool.GetSizeInBytes());
	...
	for (dgInt32 i = 0; i < m_regionsCount; i ++) {
		m_regionCom0[i] = m_regionCom0[i].Scale4 (dgFloat32 (1.0f) / m_regionMass[i]);
	}
