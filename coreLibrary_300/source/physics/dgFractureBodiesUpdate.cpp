...
dgFracturedBodiesUpdate::dgFracturedBodiesUpdate (dgMemoryAllocator* const allocator)
	:dgList<dgCollisionDeformableMesh*>(allocator)
	,m_dictionary(allocator)
{
}

void dgFracturedBodiesUpdate::AddShape(dgCollisionDeformableMesh* const shape)
{
	dgListNode* const node = Append (shape);
	m_dictionary.Insert (node, shape);
}

void dgFracturedBodiesUpdate::RemoveShape(dgCollisionDeformableMesh* const shape)
{
