...
//origin.m_y += 1.5f;
	...
	dFloat Iyy;
	dFloat Izz;
	NewtonBodyGetMass(controller->m_engine->GetBody(), &mass, &Ixx, &Iyy, &Izz);
	NewtonBodySetMassMatrix(m_body, mass * 0.25f, Ixx * 0.25f, Ixx * 0.25f, Ixx * 0.25f);
...
dCustomVehicleController::dEngineController::dEngineController (dCustomVehicleController* const controller, const dInfo& info, const dDifferential& differential)
...
dTrace (("b(%f) rate(%f)\n", m_sideSlip * 180.0f/3.1416f, (m_sideSlip - m_sideSlip0) * (180.0f / 3.1416f) / timestep));
if ((dAbs (m_sideSlip * 180.0f/3.1416f) > 35.0f))  {
	dVector xxx (matrix.m_up.Scale (-8000.0f * dSign(m_sideSlip)));
	NewtonBodyAddTorque (chassisBody, &xxx[0]);
} else {
	dFloat betaRate = (m_sideSlip - m_sideSlip0) / timestep;
