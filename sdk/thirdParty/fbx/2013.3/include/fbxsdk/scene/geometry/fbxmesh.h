...
public:
    ...
    /** Begins the process of adding a polygon to the mesh. 
      ...
      * type is \e eByPolygon. Otherwise it must be \c -1.
    ...
    /** Begin writing a polygon.
      ...
      * type is \e eByPolygon. Otherwise it must be an array of \c -1. This array is expected to be of size:
	  * FbxLayerElement::sTypeTextureCount. If one texture type is not used, the corresponding entry
	  * must be left at \c -1.
      */
    ...
    /** Add a polygon vertex to the current polygon.
      ...
      * if texture UV mapping type is \e eByPolygonVertex. Otherwise it must be \c -1.
    ...
    */
    ...
    /** Get a polygon vertex (i.e: an index to a control point).
      ...
      * The valid range for this parameter is 0 to \c FbxMesh::GetPolygonCount().
      * \param pPositionInPolygon Position of polygon vertex in indexed polygon.
      * The valid range for this parameter is 0 to \c FbxMesh::GetPolygonSize(pPolygonIndex).
    ...
    /**
	  ...
	  * quickly. Internally, they refer to \c FbxLayer and \c FbxLayerElementUV methods to do 
	  * the job. Except for the GetAllChannelUV(int pLayer), all the methods are implicitly 
	  * working on Layer 0. Use the \c FbxLayer methods to have access to the other layers.
      */
    //@{
    ...
    /**
	  ...
	  * Texture and UV indices quickly. Internally, they refer to \c FbxLayer and 
	  * \c FbxLayerElementUV methods to do the job. These functions are only working on 
	  * Layer 0. Use the \c FbxLayer methods directly to access other layers.
      */
    //@{
    ...
    /** Initialize material indices.
      ...
      * \remarks This function will set the Reference mode of the FbxLayerElementMaterial on layer 0 to \e eIndexToDirect.
      */
    ...
    /** Initialize texture indices.
      ...
      * \remarks This function will set the Reference mode of the FbxLayerElementTexture on layer 0 to \e eIndexToDirect.
      */
    ...
    /** Get a texture UV index associated with a polygon vertex (i.e: an index to a control point).
      ...
      * \remarks This function only works if the texture UV mapping mode is set to \e eByPolygonVertex,
    ...
    /** Set a texture UV index associated with a polygon vertex (i.e: an index to a control point).
	  ...
	  * \param pTypeIdentifier The texture channel the UVIndex refers to.
      * \remarks This function only works if the texture UV mapping type is set to \e eByPolygonVertex.
      */
    void SetTextureUVIndex(int pPolygonIndex, int pPositionInPolygon, int pIndex, FbxLayerElement::EType pTypeIdentifier/*=FbxLayerElement::eTextureDiffuse*/);
...
public:
    /** Generate tangents data for UVSet with specific name.
      ...
      * \param pOverwrite   If true, tangent and binormal data will be (re)generated regardless of availability.
      *                     If false, data will be left untouched if it already exists.      
      ...
      */
    bool GenerateTangentsData(FbxString pUVSetName = "", bool pOverwrite = false);
