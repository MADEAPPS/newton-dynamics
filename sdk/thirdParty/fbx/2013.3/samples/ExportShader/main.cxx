...
int main(int argc, char** argv)
{
    ...
    if(argc > 1)
    {
        // embed media file to FBX file
        lResult = SaveScene(lSdkManager, lScene, argv[1], -1, true);
    }
    // A default output file name is given otherwise.
    else
    {
        lResult = SaveScene(lSdkManager, lScene, SAMPLE_FILENAME, -1, true);
    }
...
FbxSurfaceMaterial* CreateHlslShader(FbxScene* pScene)
{
    ...
    lProp.ModifyFlag(FbxPropertyAttr::eUser, true);
    lProp.Set(0.8);
    CreateTableEntry(lTable, lProp);
