...
int main(int argc, char** argv)
{
	...
	for( int i = 1, c = argc; i < c; ++i )
	{
		...
		else if( lFilePath.Empty() ) lFilePath = argv[i];
	}

	gSceneContext = new SceneContext(!lFilePath.Empty() ? lFilePath.Buffer() : NULL, DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT, lSupportVBO);
...
void CameraSelectionCallback(int pItem)
{
    ...
    {
        gSceneContext->SetCurrentCamera(PRODUCER_PERSPECTIVE);
    }
    else if (pItem == PRODUCER_TOP_ITEM)
    {
        gSceneContext->SetCurrentCamera(PRODUCER_TOP);
    }
    else if (pItem == PRODUCER_BOTTOM_ITEM)
    {
        gSceneContext->SetCurrentCamera(PRODUCER_BOTTOM);
    }
    else if (pItem == PRODUCER_FRONT_ITEM)
    {
        gSceneContext->SetCurrentCamera(PRODUCER_FRONT);
    }
    else if (pItem == PRODUCER_BACK_ITEM)
    {
        gSceneContext->SetCurrentCamera(PRODUCER_BACK);
    }
    else if (pItem == PRODUCER_RIGHT_ITEM)
    {
        gSceneContext->SetCurrentCamera(PRODUCER_RIGHT);
    }
    else if (pItem == PRODUCER_LEFT_ITEM)
    {
        gSceneContext->SetCurrentCamera(PRODUCER_LEFT);
    }
    else if (pItem == CAMERA_SWITCHER_ITEM)
    {
        gSceneContext->SetCurrentCamera(CAMERA_SWITCHER);
    }
