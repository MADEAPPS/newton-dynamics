...
bool CreateScene(FbxScene* pScene, char* pSampleFileName)
{
    ...
    if( lMyProperty.IsValid() )
    {
        ...
        lMyProperty.ModifyFlag(FbxPropertyAttr::eAnimatable, true);
...
void MapVertexCacheOnTriangle(FbxScene* pScene, FbxNode* pTriangle, char* pSampleFileName)
{
    ...
    if (!FbxFolder::EnsureExistance(lAbsolutePCFileName))
    {
...
void AnimateVertexCacheOnTriangleDoubleVertex(FbxNode* pTriangle, double pFrameRate)
{
    ...
    {
        lRet = lCache->OpenFileForWrite(FbxCache::eMCOneFile, pFrameRate, pTriangle->GetName());
    }
    else
    {
...
void AnimateVertexCacheOnTriangleInt32(FbxNode* pTriangle, double pFrameRate)
{
    ...
    if (gExportVertexCacheMCFormat)
    {
        lRet = lCache->OpenFileForWrite(FbxCache::eMCOneFile, pFrameRate, pTriangle->GetName(), FbxCache::eInt32Array);
    }
...
void AnimateVertexCacheOnTriangleFloat(FbxNode* pTriangle, double pFrameRate)
{
    ...
    if (gExportVertexCacheMCFormat)
    {
        lRet = lCache->OpenFileForWrite(FbxCache::eMCOneFile, pFrameRate, pTriangle->GetName(), FbxCache::eFloatVectorArray);
    }
    else
    {
        lRet = false;
    ...
    while (lCurrentTime2 <= lStopTime)
    {
        ...
        if (gExportVertexCacheMCFormat)
        {
	        ...
	        {
		        FBXSDK_printf("awCache Float3 array read/write mismatch\n");
		        passTest = false;
		        break;
            }
	    }   
