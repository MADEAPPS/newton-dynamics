		...
		if (!contact) {
			...
			if (isCollidable) {
				...
				if (material->m_flags & dgContactMaterial::m_collisionEnable) {
					...
					const dgInt32 bothKinematic = body0->IsRTTIType(dgBody::m_kinematicBodyRTTI) & body1->IsRTTIType(dgBody::m_kinematicBodyRTTI);
					const dgInt32 collisionTest = !bothKinematic && (body0->IsCollidable() & body1->IsCollidable() & !(body0->m_equilibrium & body1->m_equilibrium));
...
void dgBroadPhase::UpdateRigidBodyContacts(dgBroadphaseSyncDescriptor* const descriptor, dgContactList::dgListNode* const nodePtr, dgFloat32 timeStep, dgInt32 threadID)
{
	...
	dgInt32* const activeContactsCount = &contactList->m_activeContactsCount;
