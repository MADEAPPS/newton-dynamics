	...
	if (projectFactor > dgFloat32 (0.9999f)) {
		for (dgInt32 i = 0; i < m_count; i ++) {
			...
			for (dgInt32 i = 0; i < count; i ++) {
				...
				dgAssert (error.DotProduct3(error) > dgFloat32 (1.0e-20f));
				j = i;
			}
		#endif
		...
		dgPlane plane (normal, - normal.DotProduct3(origin));
		...
		for (dgInt32 i = 0; i < m_count; i ++) {
			...
			if (side0 > dgFloat32 (0.0f)) {
				...
				if (count > 1) {
					...
					dgFloat32 error = edgeSegment.DotProduct3(edgeSegment);
	...
	if (count > 1) {
		if (maxDist < dgFloat32 (1.0e-3f)) {
			...
			for (dgInt32 i = 1; i < count; i ++) {
				proj = contactsOut[i].DotProduct3(lineDir);
		...
		if (error.DotProduct3(error) < dgFloat32 (1.0e-8f)) {
			count --;
		}
	}
...
dgInt32 dgCollisionConvexPolygon::CalculateContactToConvexHullContinue(const dgWorld* const world, const dgCollisionInstance* const parentMesh, dgCollisionParamProxy& proxy)
{
	...
	if (distance < dgFloat32(1.0f)) {
		...
		dgVector boxSize(dgVector::m_half * (hullBoxP1 - hullBoxP0));
		dgVector sphereMag2 (boxSize.DotProduct4(boxSize));
		boxSize = sphereMag2.Sqrt();

		dgVector pointInPlane (polygonMatrix.RotateVector(hullBoxP1 + hullBoxP0) * dgVector::m_half);
		dgFloat32 distToPlane = m_normal.DotProduct3(m_localPoly[0] - pointInPlane);
		...
		dgVector boxOrigin (dgVector::m_half * (boxOrigin0 + boxOrigin1)); 
		dgVector boxProjectSize (dgVector::m_half * (boxOrigin0 - boxOrigin1)); 
		...
		if (inside & !proxy.m_intersectionTestOnly) {
			...
			if (penetration < dgFloat32(0.0f)) {
				timetoImpact = penetration / relativeVelocity.DotProduct3(m_normal);
