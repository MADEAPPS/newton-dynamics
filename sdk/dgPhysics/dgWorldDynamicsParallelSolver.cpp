...
void dgParallelBodySolver::CalculateBodyForce(dgInt32 threadID)
{
	...
	for (dgInt32 l = threadID; l < bodyCount; l += step) {
		const dgInt32 i = l + 1;
		...
		forceAcc = forceAcc * dgWorkGroupFloat(startJoints->m_invWeight);
...
DG_INLINE void dgParallelBodySolver::BuildJacobianMatrix(dgJointInfo* const jointInfo, dgLeftHandSide* const leftHandSide, dgRightHandSide* const rightHandSide, dgJacobian* const internalForces)
{
	...
	for (dgInt32 i = 0; i < count; i++) {
		...
		const dgJacobian& JMinvM0 = lhs->m_JMinv.m_jacobianM0;
		const dgJacobian& JMinvM1 = lhs->m_JMinv.m_jacobianM1;
		dgVector tmpAccel(JMinvM0.m_linear * force0 + JMinvM0.m_angular * torque0 + JMinvM1.m_linear * force1 + JMinvM1.m_angular * torque1);
...
void dgParallelBodySolver::CalculateJointsForce(dgInt32 threadID)
{
	...
	for (dgInt32 i = threadID; i < jointCount; i += step) {
		...
		if (!isSleeping) {
			...
			dgVector preconditioner1 (jointInfo->m_preconditioner1 * bodyProxyArray[m1].m_weight);
			...
			for (dgInt32 k = 0; (k < 4) && (maxAccel.GetMax() > tol2); k++) {
		...
		if (m0) {
			forceM0 = forceM0.Scale(bodyProxyArray[m0].m_invWeight);
			torqueM0 = torqueM0.Scale(bodyProxyArray[m0].m_invWeight);
			dgScopeSpinPause lock(&bodyProxyArray[m0].m_lock);
			tempInternalForces[m0].m_linear += forceM0;
			tempInternalForces[m0].m_angular += torqueM0;
		}
		if (m1) {
			forceM1 = forceM1.Scale(bodyProxyArray[m1].m_invWeight);
			torqueM1 = torqueM1.Scale(bodyProxyArray[m1].m_invWeight);
			dgScopeSpinPause lock(&bodyProxyArray[m1].m_lock);
...
void dgParallelBodySolver::CalculateForces()
{
	...
	for (dgInt32 step = 0; step < 4; step++) {
		...
		dgFloat32 accNorm = DG_SOLVER_MAX_ERROR * dgFloat32(2.0f);
		for (dgInt32 i = 0; (i < passes) && (accNorm > DG_SOLVER_MAX_ERROR); i++) {
			CalculateJointsForce();
			accNorm = dgFloat32(0.0f);
			for (dgInt32 j = 0; j < threadCounts; j++) {
				accNorm += m_accelNorm[j];
			}
		}
		IntegrateBodiesVelocity();
	}
