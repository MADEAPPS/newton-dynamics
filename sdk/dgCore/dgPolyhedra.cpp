...
dgEdge* dgPolyhedra::FindEarTip (dgEdge* const face, const dgFloat64* const pool, dgInt32 stride, dgDownHeap<dgEdge*, dgFloat64>& heap, const dgBigVector &normal) const
{
	...
	d0 = d0.Scale3 (dgFloat64 (1.0f) / val);
	...
	do {
		...
		d1 = d1.Scale3 (dgFloat32 (1.0f) / val1);
...
{
	bool ret = IsEssensialPointDiagonal (diagonal, normal, pool, stride) || IsEssensialPointDiagonal (diagonal->m_twin, normal, pool, stride); 
//	if (!ret) {
//		ret = ret || ((diagonal->m_next->m_twin->m_next->m_twin == diagonal) && (diagonal->m_twin->m_next->m_twin->m_next == diagonal));
//	}
	return ret;
}
...
bool dgPolyhedra::PolygonizeFace(dgEdge* const face, const dgFloat64* const pool, dgInt32 strideInBytes)
{
	...
	if (flatFace.TriangulateFace(array[0], pool, strideInBytes)) {
		...
		if (diagonalCount) {
			...
			normal1 = normal1.Scale3(dgFloat64(1.0f) / sqrt(normal1.DotProduct3(normal1)));
