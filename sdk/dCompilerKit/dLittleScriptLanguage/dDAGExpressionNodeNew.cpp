...
void dDAGExpressionNodeNew::CompileCIL(dCIL& cil)
{
	...
	dCILInstr& arraySizeInBytes = cil.NewStatement()->GetInfo();
	arraySizeInBytes.m_instruction = dCILInstr::m_assigment;
	arraySizeInBytes.m_operator = dCILInstr::m_mul;
	arraySizeInBytes.m_arg0.m_label = cil.NewTemp();
	arraySizeInBytes.m_arg1 = m_dimension->m_result;

	arraySizeInBytes.m_arg2.m_type = dCILInstr::m_intConst;
	...
	dCILInstr& operatorNewInstruction = cil.NewStatement()->GetInfo();
	operatorNewInstruction.m_instruction = dCILInstr::m_new;
