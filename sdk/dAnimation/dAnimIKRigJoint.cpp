...
dAnimIKRigJoint::dAnimIKRigJoint(dAnimIKRigJoint* const parent)
	:dAnimAcyclicJoint(parent)
	,m_root(parent ? parent->GetRoot() : NULL)
{
}

dAnimIKRigJoint::~dAnimIKRigJoint()
{
}

/*
void dAnimIKRigJoint::UpdateLocalTransforms (dAnimIDManager* const manager) const
{
	dMatrix parentMatrixPool[128];
	const dAnimIKRigJoint* stackPool[128];
	...
	while (stack) {
		...
		const dAnimIKRigJoint* const node = stackPool[stack];
		...
		if (newtonBody) {
			...
			for (dList<dAnimAcyclicJoint*>::dListNode* child = node->m_children.GetFirst(); child; child = child->GetNext()) {
				stackPool[stack] = (dAnimIKRigJoint*) child->GetInfo();
...
void dAnimIKRigJoint::Init(NewtonBody* const newtonBody)
{
...
void dAnimIKRigJoint::RigidBodyToStates()
{
	...
	for (dList<dAnimAcyclicJoint*>::dListNode* child = m_children.GetFirst(); child; child = child->GetNext()) {
		dAnimIKRigJoint* const rigJoint = child->GetInfo()->GetAsRigJoint();
		dAssert (rigJoint);
