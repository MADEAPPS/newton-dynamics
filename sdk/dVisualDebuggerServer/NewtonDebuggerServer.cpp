...
{
	DebuggerServer* debugger = new DebuggerServer (world);

	if (!debugger->ServerStatus()) {
		delete debugger;
		debugger = NULL;
	}
	return debugger;
...
{
	delete (DebuggerServer*)server;
}


void NewtonDebuggerServe (void* const server, dFloat timestep)
{
	DebuggerServer* const debugger = (DebuggerServer*)server;
	debugger->Serve(timestep);
}