...
ValveMapImport* ValveMapImport::GetPlugin()
{
	static ValveMapImport gImporter;
	return &gImporter;
}


bool ValveMapImport::Import (const char* fileName, dScene* world)
{
...
ValveMapImport::Property* ValveMapImport::PropertyList::GetProperty (const char* name)
{
	unsigned crc = dCRC (name);

	for (ValveMapImport::PropertyList::dListNode* node = GetFirst(); node; node = node->GetNext()) {
...
/*
void ValveMapImport::AddModel (dModel* model, dScene* world)
{
...
void ValveMapImport::AddSkeleton (dBone* bone, dModel* model, dScene* world, dScene::dTreeNode* parentNode, dTree<dScene::dTreeNode*, int>& sceneNodes)
{
...
void ValveMapImport::CreateMeshGeomtry (dScene::dTreeNode* meshNode, dMesh* mesh, dScene* world)
{
	int materialID = 0;
...
void ValveMapImport::SkipChar (FILE* file, char chr)
{
	while (!feof (file) && getc (file) != chr);	
}

int ValveMapImport::NextChar (FILE* file)
{
	int ch;
...
void ValveMapImport::SkipClass (FILE* file)
{
...
void ValveMapImport::ParceMapFile (FILE* file, dScene* world, void* userData)
{	
	char className[64];
	int ch;
...
void ValveMapImport::ReadProperties (FILE* file, dList<Property>& list)
{
	int ch;
...
void ValveMapImport::LoadVersionInfo(FILE* file, dScene* world, void* userData)
{
	SkipClass (file);
}

void ValveMapImport::LoadVisGroups(FILE* file, dScene* world, void* userData)
{
	SkipClass (file);
}
...
void ValveMapImport::LoadViewSettings(FILE* file, dScene* world, void* userData)
{
	SkipClass (file);
}
...
void ValveMapImport::LoadEntity (FILE* file, dScene* world, void* userData)
{
	SkipClass (file);
}

void ValveMapImport::LoadCamera (FILE* file, dScene* world, void* userData)
{
	SkipClass (file);
}

void ValveMapImport::LoadCordon (FILE* file, dScene* world, void* userData)
{
	SkipClass (file);
}
...
void ValveMapImport::LoadSide (FILE* file, dScene* world, void* userData)
{
	SkipChar (file, '{');
...
void ValveMapImport::LoadSolid (FILE* file, dScene* world, void* userData)
{
	SkipChar (file, '{');
...
void ValveMapImport::LoadWorld (FILE* file, dScene* world, void* userData)
{
	SkipChar (file, '{');
	PropertyList propertyList;
	ReadProperties (file, propertyList);
