...
bool NewtonImport::Import (const char* const fileName, dScene* const scene, dUndoRedoManager* const undoManager)
{
	FILE* const file = fopen (fileName, "rb");

	if (file) {
		// save the state of the scene 
		undoManager->Push (new dUndoSceneGraph(scene));

		NewtonWorld* const world = NewtonCreate();
		NewtonWorldSetUserData(world, scene);
