...
public:
	...
	void ApplyInputs(dCustomPlayerController* const controller)
	{
		dFloat forwarSpeed = 0;
		dFloat strafeSpeed = 0;
		dFloat viewangle = 0;
		//
		if (controller == m_player) {
			DemoEntityManager* const scene = (DemoEntityManager*)NewtonWorldGetUserData(GetWorld());
			forwarSpeed = (int(scene->GetKeyState('W')) - int(scene->GetKeyState('S'))) * PLAYER_WALK_SPEED;
			strafeSpeed = (int(scene->GetKeyState('D')) - int(scene->GetKeyState('A'))) * PLAYER_WALK_SPEED;

			//printf("forwarSpeed: %.3f \n", forwarSpeed);
			//printf("strafeSpeed: %.3f \n", strafeSpeed);
			...
			viewangle = camera->GetYawAngle();
		}
		//
		controller->SetForwardSpeed(forwarSpeed);
		controller->SetLateralSpeed(strafeSpeed);
		controller->SetHeadingAngle(viewangle);
	}
